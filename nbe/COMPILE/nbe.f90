MODULE nbe
  ...
  private ::  set_gridvar_p
  private ::  gradx_2nd, grady_2nd, grady_cos_2nd, missing_bdy

  integer,                           private ::  nx_pre, ny_pre
  real, dimension(:),   allocatable, private ::  lat_pre, beta,          &
                                                 inv_2dx, inv_2dy
  real, dimension(:,:), allocatable, private ::  coslat, sinlat, f
...
SUBROUTINE set_gridvar_p(nx,ny,lat)
  ...
  allocate( coslat(nx,ny), sinlat(nx,ny), f(nx,ny), beta(ny),            &
  ...
  beta(:) = 2.*ome_earth*coslat(1,:)/a_earth

  inv_2dx(2:ny-1) = 1./(360./float(nx/2)*deg2rad*a_earth*coslat(1,2:ny-1))
