...
while [ $K -le $K2 ] ; do
  ...
  P990[2]=$fidir/x${D}_1hrly/${vars_f[$iv]}_z/${vars_f[$iv]}__z${ZZZZZ}__x$D$fitail
  ODIR=$fidir/x${D}_1hrly/${vars_f[$iv]}_z/prt_d300km
  P999[2]=$ODIR/prt_${vars_f[$iv]}__z${ZZZZZ}__x$D$fitail
  P999[3]=$ODIR/prt_${vars_f[$iv]}__z${ZZZZZ}__x$D$fitail
  [ -d $ODIR ] || mkdir -p $ODIR
