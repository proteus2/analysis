PROGRAM PSD_dir
  ...
  real, dimension(:,:),     allocatable ::  psd_phi_c, psd_phi_kh
  real, dimension(:,:),     allocatable ::  phie_i, phiw_i, kh_i
  real, dimension(:),       allocatable ::  kwn, lwn, ome, phi, c, kh
...
! DUMP and FINALIZE
  ...
  do j=1, nl
  do i=1, nk
    phie_i(i,j) = atan2(lwn(j),kwn(i))
    kh_i  (i,j) = sqrt(kwn(i)*kwn(i) + lwn(j)*lwn(j))
  enddo
  enddo
  phie_i(:,:) = real(phie_i(:,:)*(45.d0/atan(1.d0)))

  deg_hr2m_s = a_earth*real(atan(1.d0)/45.d0)*sqrt(1.+cos(lat_c)**2)/3600.
  do n=1, no
  do j=1, nl
  do i=1, nk
  ...
  allocate( psd_phi_c(nphi,nc), psd_phi_kh(nphi,nkh) )
  ...
  deallocate( psd_phi_c, psd_phi_kh, psd )
  deallocate( kwn, lwn, ome, phie_i, kh_i, c_i )
  deallocate( phi, c, kh )
