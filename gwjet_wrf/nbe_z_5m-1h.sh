...
fitail="__08_00_00__12_00_00__1hrly.nc"
vars_f=( U  V  PRS  T  W )  # Do not change the order !
D=6
ODIR=$fidir/x${D}_1hrly/nbe_z/mean_d300km
...
P106=( n N_SMOOTH9  3 )      # iterating number of the 9-pt local smoothing
#== Parameter 9 - I/O ============================
P990=( s FILE_I  ''  ''  ''  ''  '' )
P991=( s FILE_V_I  ''  ''  ''  '' )
...
while [ $K -le $K2 ] ; do
  ...
  i=2
  for ff in ${vars_f[@]} ; do
    P990[$i]=$fidir/x${D}_1hrly/${ff}_z/prt_d300km/prt_${ff}__z${ZZZZZ}__x$D$fitail
    i=$(( i + 1 ))
  done
  i=2
  for ff in ${vars_f[@]:0:2} ; do
    P991[$i]=$fidir/x${D}_1hrly/${ff}_z/prt_d300km/prt_${ff}__z${ZZZZZ1}__x$D$fitail
    i=$(( i + 1 ))
    P991[$i]=$fidir/x${D}_1hrly/${ff}_z/prt_d300km/prt_${ff}__z${ZZZZZ2}__x$D$fitail
    i=$(( i + 1 ))
  done
  ...
  cr_nl $F_NAMELIST FILEIO "${P990[*]}" "${P991[*]}" "${P998[*]}" "${P999[*]}"
  cat $F_NAMELIST
