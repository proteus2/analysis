...
P109=( n LAT_C  ${lat_c5[$ww]} )  # central latitude for c ([deg/hr] to [m/s]
#== Parameter 9 - I/O ============================
P990=( s FILE_I  ''  ''  '' )
P999=( s FILE_O  '' )
#=================================================
...
D=$D1  ;  while [ $D -le $D2 ] ; do
  ...
  P990[2]=$fidir/x$D/prt_d300km/fcoef_klo/fft-W${ww}_prt_W_${levs[$il]}_klo.nc
  P990[3]=$fidir/x$D/prt_d300km/fcoef_klo/fft-W${ww}_prt_U_${levs[$il]}_klo.nc
  P990[4]=$fidir/x$D/prt_d300km/fcoef_klo/fft-W${ww}_prt_V_${levs[$il]}_klo.nc
  ODIR=$fodir/x$D/prt_d300km/mfs
  P999[2]=$ODIR/mfs-W${ww}_prt_${levs[$il]}_phi_2d.nc
  [ -d $ODIR ] || mkdir -p $ODIR
