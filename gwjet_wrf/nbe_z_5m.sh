...
D=6
ODIR=$fidir/x${D}_5min/nbe_z/mean_d300km
...
P106=( n N_SMOOTH9  2 )      # iterating number of the 9-pt local smoothing
#== Parameter 9 - I/O ============================
P990=( s FILE_I  ''  ''  ''  ''  '' )
P991=( s FILE_V_I  ''  ''  ''  '' )
...
while [ $K -le $K2 ] ; do
  ...
  for ff in ${vars_f[@]:0:2} ; do
    P991[$i]=$fidir/x${D}_5min/${ff}_z/prt_d300km/mean_${ff}__z${ZZZZZ1}__x$D$fitail
    i=$(( i + 1 ))
    P991[$i]=$fidir/x${D}_5min/${ff}_z/prt_d300km/mean_${ff}__z${ZZZZZ2}__x$D$fitail
    i=$(( i + 1 ))
  done
