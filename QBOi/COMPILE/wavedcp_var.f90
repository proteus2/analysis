PROGRAM RC_VAR_RA_z
  ...
  namelist /PARAM/ VAR_NAME, K_LARGEST, PERIOD_SMALLEST, LAT_RNG, P_RNG, &
                   NMON_PATCH
  namelist /FILEIO/ MISSV, FILE_I_HEAD, FILE_I_FORM, FILE_I_XXXX,        &
                    FILE_I_HEAD2, FILE_I_FORM2, FILE_I_XXXX2,            &
                    VAR_I_NAME2, FILE_O
  ...
  real, dimension(:),         allocatable ::  lat0, p0a, kwn, ome, ome0, &
...
!  inv_pcfsum(:) = (lat0s(2)-lat0s(1))/(inv_pcfsum(:)*sqrt(2.*pi))
  ...
  deallocate( varo )
  deallocate( var4d )
  deallocate( lon, lat, p, lat0, p0a, lat0s, pcf )
