PROGRAM FFT_t2nd
! test spread in spectrum for "quasi-"monochromatic waves
...
!  fc_var_k(i_time,:,:,:) = cmplx(var_k(:,:,:,1),var_k(:,:,:,2))
  fc_var_k(i_time,:,:,:) = 0.
  fc_var_k(i_time,4,:,:) = exp((0.,1.)*float(i_time)/nhour*(6.2831853/15.))* &
                           exp(-0.5*((float(i_time)/nhour-45.)/2.)**2)
!  fc_var_k(i_time,6,:,:) = exp((0.,1.)*float(i_time)/nhour*(6.2831853/15.))* &
!                           (exp(-0.5*((float(i_time)/nhour-30.)/2.)**2) + &
!                            exp(-0.5*((float(i_time)/nhour-60.)/2.)**2))*0.5
!  fc_var_k(i_time,8,:,:) = exp((0.,1.)*float(i_time)/nhour*(6.2831853/15.))* &
!                           (exp(-0.5*((float(i_time)/nhour-30.)/2.)**2) - &
!                            exp(-0.5*((float(i_time)/nhour-60.)/2.)**2))*0.5
