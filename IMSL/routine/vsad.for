C   IMSL ROUTINE NAME   - VSAD
C
C-----------------------------------------------------------------------
C
C   LATEST REVISION     - NOVEMBER 1, 1984
C
C   PURPOSE             - SORTING OF DOUBLE PRECISION MATRICES
C                           (WITH OPTIONS)
C
C   USAGE               - CALL VSAD (A,IA,NR,NC,IOP,KPOS,NK,IR,WK,IER)
C
C   ARGUMENTS    A      - INPUT/OUTPUT NR BY NC DOUBLE PRECISION ARRAY
...
C                WK     - DOUBLE PRECISION WORK VECTOR OF LENGTH IIW,
C                           WHERE IIW = (1-IOP(1))*MAX(NC,2*NR) +
C                             IOP(1)*MAX(NR,2*NC).
...
C                               LIMITED TO IA.
C
C   REQD. IMSL ROUTINES - UERTST,UGETIO,VBLA=IDAMAX,VBLA=DASUM,
C                           VBLA=DCOPY,VBLA=DNRM2,VBLA=DSWAP,VNABSX,
C                           VNAXPB,VNDIST,VNINI,VNPIVD,VNDCPY,VNSWAP,
C                           VSARER,VSADX,VDCMP,VDCMPA,VDCMPE,VSENA,
C                           VSIDA,VSIDAQ,VSIDE,VSIDEQ,VSIDU,VSIDUQ,
C                           VSODA,VSODAQ,VSODE,VSODEQ,VSODU,VSODUQ,
C                           VSRTPD,VDSWAM
C
...
C-----------------------------------------------------------------------
C
      SUBROUTINE VSAD (A,IA,NR,NC,IOP,KPOS,NK,IR,WK,IER)
C                                  SPECIFICATIONS FOR ARGUMENTS
      INTEGER            IA,NR,NC,NK,IER,IOP(1),KPOS(1),IR(1)
      DOUBLE PRECISION   A(IA,1),WK(1)
...
C                                  ERROR CHECKING AND INITIALIZATION
      ...
      CALL VSADX(A,IA,LR,LC,1-IOP(1),IR(IK),WK)
      N = 5*IOP(1)+IOP(2)+1
      GO TO (5, 10, 15, 15, 15, 20, 25, 30, 30, 30), N
C                                  SORT COLUMNS
    5 IF (IOP(4).EQ.0) CALL VSODA(A,IA,LR,LC,LK,WK,IE)
      IF (IOP(4).GT.0) CALL VSODAQ(A,IA,LR,LC,LK,IR,WK,IE)
      GO TO 35
   10 IF (IOP(4).EQ.0) CALL VSODU(A,IA,LR,LC,LK,WK,IE)
      IF (IOP(4).GT.0) CALL VSODUQ(A,IA,LR,LC,LK,IR,WK,IE)
      GO TO 35
   15 IF (IOP(4).EQ.0) CALL VSODE(A,IA,LR,LC,IOP(2)-1,LK,WK,IE)
      IF (IOP(4).GT.0) CALL VSODEQ(A,IA,LR,LC,IOP(2)-1,LK,IR,WK,IE)
      GO TO 35
C                                  SORT ROWS
   20 IF (IOP(4).EQ.0) CALL VSIDA(A,IA,LR,LC,LK,WK,IE)
      IF (IOP(4).GT.0) CALL VSIDAQ(A,IA,LR,LC,LK,IR,WK,IE)
      GO TO 35
   25 IF (IOP(4).EQ.0) CALL VSIDU(A,IA,LR,LC,LK,WK,IE)
      IF (IOP(4).GT.0) CALL VSIDUQ(A,IA,LR,LC,LK,IR,WK,IE)
      GO TO 35
   30 IF (IOP(4).EQ.0) CALL VSIDE(A,IA,LR,LC,IOP(2)-1,LK,WK,IE)
      IF (IOP(4).GT.0) CALL VSIDEQ(A,IA,LR,LC,IOP(2)-1,LK,IR,WK,IE)
C                                  DESCENDING/DETACHED/DISTINCT CASES
   35 CALL VNPIVD(IR(IK),LJR,WK)
      CALL VSADX(A,IA,LR,LC,1-IOP(1),IR(IK),WK)
     ...
     *VDSWAM(A,IA,LR,LC,IOP(1),1,1)
      IF (IOP(4)*IOP(3).GE.3) CALL VNAXPB(ND,-1,IR(ID),1,LIR+1,0)
      IF (IOP(4).EQ.2 .OR. IOP(4).EQ.5) CALL VNPIVD(IR,LIR,WK)
      IF (IOP(4).EQ.2 .OR. IOP(4).EQ.5) CALL VSADX(A,IA,LR,LC,IOP(1),IR,
     *WK)
      IF (IOP(4).EQ.2 .OR. IOP(4).EQ.5) CALL VNPIVD(IR,LIR,WK)
 ...
 9000 CONTINUE
      IF (IER.NE.0) CALL UERTST(IER,6HVSAD  )
 9005 RETURN
      END
