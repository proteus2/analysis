...
C
C   USAGE               - CALL USPLO (X,Y,IY,N,M,INC,ITITLE,NTITLE,
C                           IXLABL,NXLABL,IYLABL,NYLABL,RANGE,ICHAR,
C                           IOPT,IER)
C
C   ARGUMENTS    X      - VECTOR OF LENGTH N CONTAINING THE
C                           INDEPENDENT VARIABLE VALUES. (INPUT)
C                Y      - MATRIX OF DIMENSION N BY M CONTAINING
C                           THE M SETS OF FUNCTION VALUES. (INPUT)
...
C                           TO BE USED. USPLO PLOTS X(1+(I-1)*INC) FOR
C                           I=1,...N. (INPUT)
...
C                RANGE  - VECTOR OF LENGTH 4 SPECIFYING MINIMUM AND
C                           MAXIMUM RANGES FOR X,Y AXES. (INPUT) (MIN X,
C                           MAX X, MIN Y, MAX Y, RESPECTIVELY). USPLO
...
C-----------------------------------------------------------------------
C
      SUBROUTINE USPLO (X,Y,IY,N,M,INC,ITITLE,NTITLE,IXLABL,NXLABL,
...
C                                  SPECIFICATIONS FOR LOCAL VARIABLES
     ...
     *                   NCHMTB,NLEN,NLOG,NMIN,NOUT,PLUS,STAR,SYM4(10)
      REAL               A,H,HTRY(6),SFX,SFY,TEMP,X1,X2,XLEN,XMAX,XMIN,
     *                   XNOT(6),Y1,Y2,YMAX,YMIN,YNOT
      DATA               MP2/1HM/,IYS/1HI/,IXS/1H-/,STAR/1H*/,PLUS/1H+/
      DATA               BLNK4 /1H /,HTRY /1.,2.,4.,5.,8.,10./
...
C
   45 XLEN = IXD/10.
      ...
      DO 50 I=1,6
         ...
         A = NMIN*H
         IF ((XMAX-A).LE.(XLEN+0.01)*H) GO TO 55
   50 CONTINUE
   55 XMIN = A
      XMAX = A+XLEN*H
C                                  FIND MIN,MAX OF Y
...
C                                  IDENTICAL POINTS BECOME M
  ...
  230    FORMAT (1X, A1, 1X, E10.2, 1X, 101A1)
  235    CONTINUE
  240 CONTINUE
      X1 = 1./SFX
      X2 = XMIN-X1
      ...
      DO 245 I=1,IXDP1,20
         XNOT(L) = I*X1+X2
         IF (ABS(XNOT(L)).LT.X1) XNOT(L) = 0.0
         L = L+1
  245 CONTINUE
