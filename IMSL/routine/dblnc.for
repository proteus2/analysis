...
C-----------------------------------------------------------------------
C
      REAL FUNCTION DBLNC (F,A,B,AERR,RERR,ERROR,IER)
C                                  SPECIFICATIONS FOR ARGUMENTS
      INTEGER            IER
      REAL               F,A,B,AERR,RERR,ERROR
C                                  SPECIFICATIONS FOR LOCAL VARIABLES
      ...
      REAL               TWO,FOUR,FOURP5,TEN,HUN,CADRE,AITLOW
...
C                                    IS THE FIRST INTERVAL CONSIDERED.
      BEG = A
      FBEG = DBLND(F,BEG)*HALF
      TS(1) = FBEG
      IBEG = 1
      EDN = B
      FEND = DBLND(F,EDN)*HALF
      TS(2) = FEND
      IEND = 2
    5 RIGHT = .FALSE.
...
C                                    IF TEST IS PASSED, ACCEPT *VINT*
  135 SLOPE = (FEND-FBEG)*TWO
      FBEG2 = FBEG+FBEG
      DO 140 I=1,4
         DIFF = ABS(DBLND(F,BEG+RN(I)*STEP) - FBEG2-RN(I)*SLOPE)
         HRERR = TABTLM+DIFF
...
C                                    ACCEPT *VINT*
  145 SLOPE = (FEND-FBEG)*TWO
      FBEG2 = FBEG+FBEG
      I = 1
  150 DIFF = ABS(DBLND(F,BEG+RN(I)*STEP) - FBEG2-RN(I)*SLOPE)
...
C                                    TION PROBLEM
  ...
  220 DBLNC = CADRE
 9000 CONTINUE
      IF (IER .NE. 0) CALL UERTST (IER,6HDBLNC )
 9005 RETURN
      END
