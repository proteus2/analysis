C   IMSL ROUTINE NAME   - VSRTUD
C
C-----------------------------------------------------------------------
C
C   LATEST REVISION     - JANUARY 1, 1978
C
C   PURPOSE             - INTERCHANGE THE ROWS OR COLUMNS OF A DOUBLE
C                           PRECISION MATRIX USING A PERMUTATION VECTOR
C                           SUCH AS THE ONE OBTAINED FROM IMSL ROUTINES
C                           VSRTPD OR VSRTRD
C
C   USAGE               - CALL VSRTUD (Z,IZ,N,M,IND,IR,WK)
C
C   ARGUMENTS    Z      - INPUT DOUBLE PRECISION MATRIX OF DIMENSION
C                           N BY M TO BE INTERCHANGED.  ON OUTPUT, Z
C                           CONTAINS THE DOUBLE PRECISION INTERCHANGED
C                           MATRIX.
...
C-----------------------------------------------------------------------
C
      SUBROUTINE VSRTUD (Z,IZ,N,M,IND,IR,WK)
C
      INTEGER            IR(1)
      DOUBLE PRECISION   Z(IZ,1),WK(1)
