...
C-----------------------------------------------------------------------
C
      SUBROUTINE VSODEQ (A,IA,NR,NC,IOPT,NK,IR,WK,IER)
...
C                                    WK
      CALL VDCMPE(IOPT,KR,A(IROWA,I),INKA,WK(IROWA),INKW,KOMPAR)
...
C                                    THAN VECTOR WK, INTERCHANGE WITH
C                                    WK
      CALL VDCMPE(IOPT,KR,A(IROWA,J),INKA,WK(IROWA),INKW,KOMPAR)
...
C                                    WK
      ...
      GO TO 45
   40 CONTINUE
      CALL VDCMPE(IOPT,KR,A(IROWA,L),INKA,A(IROWA,K),INKA,KOMPAR)
...
C                                    THE MATRIX WHICH IS LESS THAN WK.
   45 CONTINUE
      L = L-1
      CALL VDCMPE(IOPT,KR,A(IROWA,L),INKA,WK(IROWA),INKW,KOMPAR)
...
C                                    WK.
   50 CONTINUE
      K = K+1
      CALL VDCMPE(IOPT,KR,A(IROWA,K),INKA,WK(IROWA),INKW,KOMPAR)
...
C                                    THE UNSORTED MATRIX.
   ...
   75 CONTINUE
      ...
      IR(K+1) = IR(K)
      K = K-1
      CALL VDCMPE(IOPT,KR,WK(IROWA),INKW,A(IROWA,K),INKA,KOMPAR)
      IF (KOMPAR.EQ.-1) GO TO 75
      CALL DCOPY(NR1,WK(1),INCW,A(1,K+1),INCA)
      IR(K+1) = IT
      GO TO 70
C                                  DETERMINE THE DISTINCT COLUMNS OF
C                                    THE SORTED MATRIX.
   80 IT = 0
      IF (NC.LT.2) GO TO 90
      DO 85 I=2,NC
         IJ = NC-I+2
         CALL VDCMPE(IOPT,KR,A(IROWA,IJ-1),INKA,A(IROWA,IJ),INKA,KOMPAR)
   ...
   90 IR(1) = -IR(1)
 9000 CONTINUE
      IF (IER.NE.0) CALL UERTST(IER,6HVSODEQ)
 9005 RETURN
      END
