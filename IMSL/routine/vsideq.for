...
C
C   REQD. IMSL ROUTINES - UERTST,UGETIO,VDCMPE,VBLA=DCOPY
C
...
C-----------------------------------------------------------------------
C
      SUBROUTINE VSIDEQ (A,IA,NR,NC,IOPT,NK,IR,WK,IER)
C                                  SPECIFICATIONS FOR ARGUMENTS
      INTEGER            IA,NR,NC,IOPT,NK,IER,IR(1)
...
C                                  SPECIFICATIONS FOR LOCAL VARIABLES
     ...
     *                   JCOLA,JCOLB,K,KC,KOMPAR,KR,L,M,NC1,NK1,NR1
      REAL               DR,R,R1,UR
...
C                                    MATRIX A AND SAVE IT IN VECTOR WK.
      IJ = I+(J-I)*R
      CALL DCOPY(NC1,A(IJ,1),INCA,WK(1),INCW)
      IT = IR(IJ)
C                                  IF 1ST ROW OF MATRIX IS GREATER
C                                    THAN VECTOR WK INTERCHANGE WITH WK.
      CALL VDCMPE(IOPT,KC,A(I,JCOLA),INKA,WK(JCOLA),INKW,KOMPAR)
      IF (KOMPAR.EQ.-1 .OR. KOMPAR.EQ.0) GO TO 35
      CALL DCOPY(NC1,A(I,1),INCA,A(IJ,1),INCA)
      CALL DCOPY(NC1,WK(1),INCW,A(I,1),INCA)
      CALL DCOPY(NC1,A(IJ,1),INCA,WK(1),INCW)
      IR(IJ) = IR(I)
      IR(I) = IT
      IT = IR(IJ)
...
C                                    THAN WK INTERCHANGE WITH WK.
      CALL VDCMPE(IOPT,KC,A(J,JCOLA),INKA,WK(JCOLA),INKW,KOMPAR)
      IF (KOMPAR.EQ.1 .OR. KOMPAR.EQ.0) GO TO 45
      CALL DCOPY(NC1,A(J,1),INCA,A(IJ,1),INCA)
      CALL DCOPY(NC1,WK(1),INCW,A(J,1),INCA)
      CALL DCOPY(NC1,A(IJ,1),INCA,WK(1),INCW)
      IR(IJ) = IR(J)
      IR(J) = IT
      IT = IR(IJ)
...
C                                    THAN VECTOR WK INTERCHANGE WITH WK.
      ...
      GO TO 45
   40 CONTINUE
      CALL VDCMPE(IOPT,KC,A(L,JCOLA),INKA,A(K,JCOLA),INKA,KOMPAR)
      IF (KOMPAR.EQ.0) GO TO 45
      CALL DCOPY(NC1,A(L,1),INCA,WK(NC1+1),INCW)
      CALL DCOPY(NC1,A(K,1),INCA,A(L,1),INCA)
      CALL DCOPY(NC1,WK(NC1+1),INCW,A(K,1),INCA)
      ITT = IR(L)
      IR(L) = IR(K)
      IR(K) = ITT
...
C                                    WK.
   45 CONTINUE
      L = L-1
      CALL VDCMPE(IOPT,KC,A(L,JCOLA),INKA,WK(JCOLA),INKW,KOMPAR)
...
C                                    WK.
   50 CONTINUE
      K = K+1
      CALL VDCMPE(IOPT,KC,A(K,JCOLA),INKA,WK(JCOLA),INKW,KOMPAR)
...
C                                    THE UNSORTED MATRIX.
   ...
   70 CONTINUE
      I = I+1
      IF (I.EQ.J) GO TO 60
      CALL DCOPY(NC1,A(I+1,1),INCA,WK(1),INCW)
      IT = IR(I+1)
      CALL VDCMPE(IOPT,KC,A(I,JCOLA),INKA,WK(JCOLA),INKW,KOMPAR)
      IF (KOMPAR.EQ.-1 .OR. KOMPAR.EQ.0) GO TO 70
      K = I
   75 CONTINUE
      CALL DCOPY(NC1,A(K,1),INCA,A(K+1,1),INCA)
      IR(K+1) = IR(K)
      K = K-1
      CALL VDCMPE(IOPT,KC,WK(JCOLA),INKW,A(K,JCOLA),INKA,KOMPAR)
      IF (KOMPAR.EQ.-1) GO TO 75
      CALL DCOPY(NC1,WK(1),INCW,A(K+1,1),INCA)
      IR(K+1) = IT
      GO TO 70
C                                  DETERMINE THE DISTINCT ROWS OF
C                                    THE SORTED MATRIX.
   80 IT = 0
      IF (NR1.LT.2) GO TO 90
      DO 85 I=2,NR1
         IJ = NR1-I+2
         CALL VDCMPE(IOPT,KC,A(IJ-1,JCOLA),INKA,A(IJ,JCOLA),INKA,KOMPAR)
         IF (KOMPAR.EQ.0) GO TO 85
         IR(IJ) = -IR(IJ)
         IT = IT+1
   85 CONTINUE
   90 IR(1) = -IR(1)
 9000 CONTINUE
      IF (IER.NE.0) CALL UERTST(IER,6HVSIDEQ)
 9005 RETURN
      END
