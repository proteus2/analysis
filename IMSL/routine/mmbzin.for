...
C                           NONNEGATIVE INTEGER ORDER FOR COMPLEX
C                           ARGUMENTS
C
C   USAGE               - CALL MMBZIN (X,Y,N,BR,BI,IER)
C
C   ARGUMENTS    X      - INPUT REAL PART OF THE COMPLEX ARGUMENT.
C                           X MUST BE LESS THAN EXPARG IN ABSOLUTE
...
C-----------------------------------------------------------------------
C
      SUBROUTINE MMBZIN (X,Y,N,BR,BI,IER)
C                                  SPECIFICATIONS FOR ARGUMENTS
      INTEGER            N,IER
      DOUBLE PRECISION   X,Y,BR(N),BI(N)
C                                  SPECIFICATIONS FOR LOCAL VARIABLES
      INTEGER            NCALC,MAGZ,M,NTEN,NSTART,NEND,MRECUR,K,L,NN,
     *                   JMAG,ITEMP,NBMZ,LARGEZ,I,IMRECR,IPOS,MLAST
      DOUBLE PRECISION   PR,PI,PLASTR,PLASTI,POLDR,POLDI,PSAVER,PSAVEI,
     *                   EXPARG,TEST,TOVER,TEMPAR,TEMPAI,TEMPBR,TEMPBI,
     *                   TEMPCR,TEMPCI,RSIGN,SUMR,SUMI,ZINVR,ZINVI,XINF
      DOUBLE PRECISION   RTEN,DSIG,RNTEN,SRPRI
      DATA               DSIG/16.0D0/
      DATA               RNTEN/75.D0/
      DATA               LARGEZ /10000/
      DATA               EXPARG/174.0D0/
...
C                                  FIRST EXECUTABLE STATEMENT
      ...
      IF(N.GT.0 .AND. MAGZ.LE.LARGEZ .AND. (DABS(X).LE.EXPARG))GO TO 10
...
C                                    SMALL Z
      IF(TEMPAR**4 .LT. .1D0**DSIG) GO TO 175
...
C                                  CALCULATE P*S UNTIL SIGNIFICANCE TEST
C                                    IS PASSED.
   30 NN = NN+1
      POLDR = PLASTR
      POLDI = PLASTI
...
C                                    UNTIL UNNORMALIZED P WOULD OVERFLOW
   ...
   40 NN = NN+1
      ...
      PR = RSIGN*(DBLE(FLOAT(2*NN))*(PLASTR*ZINVR-PLASTI*ZINVI)-POLDR)
      PI = RSIGN*(DBLE(FLOAT(2*NN))*(PLASTI*ZINVR+PLASTR*ZINVI)-POLDI)
      IF(PR**2+PI**2.LE.TEST) GO TO 40
C                                  CALCULATE BACKWARD TEST, AND FIND
C                                    NCALC, THE HIGHEST N SUCH THAT THE
C                                    TEST IS PASSED.
   ...
   45 CONTINUE
      NCALC = NEND+1
   50 NCALC = NCALC-1
C                                  THE COEFFICIENT OF B(N) IN THE
C                                    NORMALIZATION SUM IS
C                                    M*SQRT(-1)**JMAG, WHERE M = -2,0,
C                                    OR 2, AND JMAG IS 0 OR 1. CALCULATE
...
C                                    INTITALIZE THEM.
   ...
   65 MRECUR = 4*((3+IPOS)/2)-3-2*(1+IPOS)
      K = 2+IPOS+2*IPOS**2-1
      L = NN-4*(NN/4)
...
C                                  TWO-TERM ASCENDING SERIES FOR SMALL Z
  ...
  180 CONTINUE
  185 IF(N.EQ.NCALC) GO TO 9005
      IER = 129+NCALC
 9000 CONTINUE
      CALL UERTST(IER,6HMMBZIN)
 9005 RETURN
      END
