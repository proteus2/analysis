...
C-----------------------------------------------------------------------
C
      SUBROUTINE VSORUQ (A,IA,NR,NC,NK,IR,WK,IER)
...
C                                    WK
      CALL VSCMPA(KR,A(IROWA,I),INKA,WK(IROWA),INKW,KOMPAR)
...
C                                    THAN VECTOR WK, INTERCHANGE WITH
C                                    WK
      CALL VSCMPA(KR,A(IROWA,J),INKA,WK(IROWA),INKW,KOMPAR)
...
C                                    WK
      ...
      GO TO 45
   40 CONTINUE
      CALL VSCMPA(KR,A(IROWA,L),INKA,A(IROWA,K),INKA,KOMPAR)
...
C                                    THE MATRIX WHICH IS LESS THAN WK.
   45 CONTINUE
      L = L-1
      CALL VSCMPA(KR,A(IROWA,L),INKA,WK(IROWA),INKW,KOMPAR)
...
C                                    WK.
   50 CONTINUE
      K = K+1
      CALL VSCMPA(KR,A(IROWA,K),INKA,WK(IROWA),INKW,KOMPAR)
...
C                                    THE UNSORTED MATRIX.
   ...
   75 CONTINUE
      ...
      IR(K+1) = IR(K)
      K = K-1
      CALL VSCMPA(KR,WK(IROWA),INKW,A(IROWA,K),INKA,KOMPAR)
...
C                                    THE SORTED MATRIX.
   80 IT = 0
      ...
      DO 85 I=2,NC
         IJ = NC-I+2
         CALL VSCMPA(KR,A(IROWA,IJ-1),INKA,A(IROWA,IJ),INKA,KOMPAR)
   ...
   90 IR(1) = -IR(1)
 9000 CONTINUE
      IF (IER.NE.0) CALL UERTST(IER,6HVSORUQ)
 9005 RETURN
      END
