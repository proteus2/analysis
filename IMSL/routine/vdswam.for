...
C
C
C   REQD. IMSL ROUTINES - VBLA=DSWAP
C
...
C-----------------------------------------------------------------------
C
      SUBROUTINE VDSWAM(A,IA,NR,NC,IOP,INCR,INCC)
C                                  SPECIFICATIONS FOR ARGUMENTS
      INTEGER            IA,NR,NC,IOP,INCR,INCC
      DOUBLE PRECISION   A(IA,1)
...
C                                    INCREMENTS.
    5 IX = 1
      ...
      IF (NH.EQ.0) GO TO 15
      DO 10 I = 1, NH
        CALL DSWAP(NR,A(1,IX),INCR,A(1,NC-IX+1),INCR)
        IX = IX + INCC
   10 CONTINUE
   15 RETURN
...
C                                    OF COLUMNS A MULTIPLE OF 3.
   20 NH = NC/2
      ...
      IF (M.EQ.0) GO TO 30
      DO 25 I = 1, M
        CALL DSWAP(NR,A(1,I),INCR,A(1,NC-I+1),INCR)
   25 CONTINUE
      IF (NH.LT.3) RETURN
   30 MP1 = M + 1
      IF (NH.EQ.0) GO TO 40
      DO 35 I = MP1, NH, 3
        CALL DSWAP(NR,A(1,I),INCR,A(1,NC-I+1),INCR)
        CALL DSWAP(NR,A(1,I+1),INCR,A(1,NC-I),INCR)
        CALL DSWAP(NR,A(1,I+2),INCR,A(1,NC-I-1),INCR)
   35 CONTINUE
   40 RETURN
   45 CONTINUE
...
C                                    INCREMENTS.
   60 IX = 1
      ...
      IF (NH.EQ.0) GO TO 70
      DO 65 I = 1, NH
        CALL DSWAP(NC,A(IX,1),INCA,A(NR-IX+1,1),INCA)
        IX = IX + INCR
   65 CONTINUE
   70 RETURN
C                                  CODE FOR BOTH INCREMENTS EQUAL TO 1
...
C                                    OF COLUMNS A MULTIPLE OF 3.
   75 NH = NR/2
      ...
      IF (M.EQ.0) GO TO 85
      DO 80 I = 1, M
        CALL DSWAP(NC,A(I,1),INCA,A(NR-I+1,1),INCA)
   80 CONTINUE
      IF (NH.LT.3) RETURN
   85 MP1 = M + 1
      IF (NH.EQ.0) GO TO 95
      DO 90 I = MP1, NH, 3
        CALL DSWAP(NC,A(I,1),INCA,A(NR-I+1,1),INCA)
        CALL DSWAP(NC,A(I+1,1),INCA,A(NR-I,1),INCA)
        CALL DSWAP(NC,A(I+2,1),INCA,A(NR-I-1,1),INCA)
   90 CONTINUE
   95 RETURN
  100 CONTINUE
C                                  CODE FOR EQUAL, POSITIVE, NONUNIT
