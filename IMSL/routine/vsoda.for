C   IMSL ROUTINE NAME   - VSODA
C
C-----------------------------------------------------------------------
C
C   LATEST REVISION     - JUNE 1, 1982
C
C   PURPOSE             - SORTING OF COLUMNS OF A DOUBLE PRECISION
C                           MATRIX IN ASCENDING ORDER OF KEYS IN ROWS.
C
C   USAGE               - CALL VSODA (A,IA,NR,NC,NK,WK,IER)
C
...
C                             LEAST SIGNIFICANT.
C                WK     - DOUBLE PRECISION WORK VECTOR OF LENGTH (2*NR).
...
C
C   REQD. IMSL ROUTINES - UERTST,UGETIO,VDCMP,VBLA=DCOPY
C
...
C                  2.  FOR J=2,...,NK, IF A(I,K).EQ.A(I,K+1)
...
C                                    IN VECTOR WK.
      IJ = I+(J-I)*R
      CALL DCOPY(NR1,A(1,IJ),INCA,WK(1),INCW)
C                                  IF 1ST COLUMN OF MATRIX IS GREATER
C                                    THAN VECTOR WK, INTERCHANGE WITH
C                                    WK
      CALL VDCMP(KR,A(IROWA,I),INKA,WK(IROWA),INKW,KOMPAR)
      IF (KOMPAR.EQ.-1 .OR. KOMPAR.EQ.0) GO TO 35
      CALL DCOPY(NR1,A(1,I),INCA,A(1,IJ),INCA)
      CALL DCOPY(NR1,WK(1),INCW,A(1,I),INCA)
      CALL DCOPY(NR1,A(1,IJ),INCA,WK(1),INCW)
   35 CONTINUE
      L = J
...
C                                    THAN VECTOR WK, INTERCHANGE WITH
C                                    WK
      CALL VDCMP(KR,A(IROWA,J),INKA,WK(IROWA),INKW,KOMPAR)
      IF (KOMPAR.EQ.1 .OR. KOMPAR.EQ.0) GO TO 45
      CALL DCOPY(NR1,A(1,J),INCA,A(1,IJ),INCA)
      CALL DCOPY(NR1,WK(1),INCW,A(1,J),INCA)
      CALL DCOPY(NR1,A(1,IJ),INCA,WK(1),INCW)
C                                  IF 1ST COLUMN OF MATRIX IS GREATER
C                                    THAN VECTOR WK, INTERCHANGE WITH
C                                    WK
      ...
      GO TO 45
   40 CONTINUE
      CALL VDCMP(KR,A(IROWA,L),INKA,A(IROWA,K),INKA,KOMPAR)
      IF (KOMPAR.EQ.0) GO TO 45
      CALL DCOPY(NR1,A(1,L),INCA,WK(NR1+1),INCW)
      CALL DCOPY(NR1,A(1,K),INCA,A(1,L),INCA)
      CALL DCOPY(NR1,WK(NR1+1),INCW,A(1,K),INCA)
C                                  FIND A COLUMN IN THE SECOND HALF OF
C                                    THE MATRIX WHICH IS LESS THAN WK.
   45 CONTINUE
      L = L-1
      CALL VDCMP(KR,A(IROWA,L),INKA,WK(IROWA),INKW,KOMPAR)
...
C                                    WK.
   50 CONTINUE
      K = K+1
      CALL VDCMP(KR,A(IROWA,K),INKA,WK(IROWA),INKW,KOMPAR)
...
C                                    THE UNSORTED MATRIX.
   ...
   70 CONTINUE
      I = I+1
      IF (I.EQ.J) GO TO 60
      CALL DCOPY(NR1,A(1,I+1),INCA,WK(1),INCW)
      CALL VDCMP(KR,A(IROWA,I),INKA,WK(IROWA),INKW,KOMPAR)
      IF (KOMPAR.EQ.-1 .OR. KOMPAR.EQ.0) GO TO 70
      K = I
   75 CONTINUE
      CALL DCOPY(NR1,A(1,K),INCA,A(1,K+1),INCA)
      K = K-1
      CALL VDCMP(KR,WK(IROWA),INKW,A(IROWA,K),INKA,KOMPAR)
      IF (KOMPAR.EQ.-1) GO TO 75
      CALL DCOPY(NR1,WK(1),INCW,A(1,K+1),INCA)
      GO TO 70
 9000 CONTINUE
      IF (IER.NE.0) CALL UERTST(IER,6HVSODA )
 9005 RETURN
      END
