C   IMSL ROUTINE NAME   - VBLA=CZDOTC
C
C-----------------------------------------------------------------------
C
...
C                           CONJUGATED VECTOR COMPONENTS (AND
C                           DOUBLE PRECISION ACCUMULATION)
C
C   USAGE               - FUNCTION CZDOTC(N,CX,INCX,CY,INCY)
C
C   ARGUMENTS    CZDOTC - COMPLEX SUM FROM I=1 TO N OF CONJ(X(I))*Y(I).
C                           (OUTPUT)
...
C                                  FIRST EXECUTABLE STATEMENT
      ...
      DO 5 I=1,N
         ...
         KY = KY+INCY
    5 CONTINUE
      CZDOTC = CMPLX(SNGL(SUMR),SNGL(SUMI))
      RETURN
   10 CONTINUE
      NS = N*INCX
      DO 15 I=1,NS,INCX
