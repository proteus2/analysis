...
C                           SEARCH METHOD - DATA PARAMETERS SPECIFIED
C
C   USAGE               - CALL ZXGSP (F,P1,P2,IP3,IP4,IP5,A,B,TOL,XMIN,
C                           IER)
C
...
C                           FORM
C                             F(X,P1,P2,IP3,IP4,IP5)
C                           WHERE X IS THE INDEPENDENT VARIABLE.
C                           P1,P2,IP3,IP4, AND IP5 ARE DESCRIBED BELOW.
...
C                                F(X1).
C                P1     - PARAMETERS THAT MAY BE USED TO SEND DATA TO
C                P2         OR RETURN DATA FROM THE FUNCTION, F.
C                IP3        (INPUT/OUTPUT)
C                IP4        P1, P2, AND IP3 ARE VECTORS. IP4 AND IP5
C                IP5        ARE SCALERS.
C                A      - (INPUT/OUTPUT)
...
C
C   REMARKS  1.  THE ARGUMENTS P1,P2,IP3,IP4, AND IP5 ARE NOT
C                USED BY THE ROUTINE. THESE ARE PARAMETERS WHICH THE
C                USER MAY REQUIRE TO RELAY DATA FROM THE MAIN PROGRAM
C                TO THE FUNCTION SUBPROGRAM F. THE CHOICE OF FIVE
C                COMMUNICATION PARAMETERS WAS NOT ENTIRELY ARBITRARY.
C                FOR EXAMPLE, FIVE PARAMETERS ALLOW THE USE OF
C                  A. A REAL 3-DIMENSIONAL ARRAY
C                  B. A REAL VECTOR
C                  C. AN INTEGER VECTOR
C                  D. AN INTEGER SCALAR SPECIFYING THE FIRST DIMENSION
C                     OF THE ARRAY EXACTLY AS IT APPEARS IN THE MAIN
C                     PROGRAM
C                  E. AN INTEGER SCALAR SPECIFYING THE SECOND DIMENSION
C                     OF THE ARRAY EXACTLY AS IT APPEARS IN THE MAIN
C                     PROGRAM
C                THIS FEATURE ALLOWS THE USER TO DEFINE A FUNCTION OF
C                SOMEWHAT GENERAL APPLICABILITY WITHOUT THE USE OF
C                COMMON.
C            2.  IMSL SUBROUTINE ZXGSN IS IDENTICAL IN
C                PURPOSE TO ZXGSP BUT DOES NOT REQUIRE INPUT
C                PARAMETERS P1,P2,IP3,IP4, AND IP5.
C
C-----------------------------------------------------------------------
C
      SUBROUTINE ZXGSP  (F,P1,P2,IP3,IP4,IP5,A,B,TOL,XMIN,IER)
C                                  SPECIFICATIONS FOR ARGUMENTS
      INTEGER            IP3(1),IP4,IP5,IER
      REAL               F,P1(1),P2(1),A,B,TOL,XMIN
...
C                                  MAKE THE INITIAL FUNCTION EVALUATIONS
      FA = F(A,P1,P2,IP3,IP4,IP5)
      FB = F(B,P1,P2,IP3,IP4,IP5)
      FV1 = F(V1,P1,P2,IP3,IP4,IP5)
      FV2 = F(V2,P1,P2,IP3,IP4,IP5)
