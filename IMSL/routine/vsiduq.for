...
C
C   REQD. IMSL ROUTINES - UERTST,UGETIO,VDCMPA,VBLA=DCOPY
C
...
C-----------------------------------------------------------------------
C
      SUBROUTINE VSIDUQ (A,IA,NR,NC,NK,IR,WK,IER)
...
C                                    THAN VECTOR WK INTERCHANGE WITH WK.
      CALL VDCMPA(KC,A(I,JCOLA),INKA,WK(JCOLA),INKW,KOMPAR)
...
C                                    THAN WK INTERCHANGE WITH WK.
      CALL VDCMPA(KC,A(J,JCOLA),INKA,WK(JCOLA),INKW,KOMPAR)
...
C                                    THAN VECTOR WK INTERCHANGE WITH WK.
      ...
      GO TO 45
   40 CONTINUE
      CALL VDCMPA(KC,A(L,JCOLA),INKA,A(K,JCOLA),INKA,KOMPAR)
...
C                                    WK.
   45 CONTINUE
      L = L-1
      CALL VDCMPA(KC,A(L,JCOLA),INKA,WK(JCOLA),INKW,KOMPAR)
...
C                                    WK.
   50 CONTINUE
      K = K+1
      CALL VDCMPA(KC,A(K,JCOLA),INKA,WK(JCOLA),INKW,KOMPAR)
...
C                                    THE UNSORTED MATRIX.
   ...
   75 CONTINUE
      ...
      IR(K+1) = IR(K)
      K = K-1
      CALL VDCMPA(KC,WK(JCOLA),INKW,A(K,JCOLA),INKA,KOMPAR)
...
C                                    THE SORTED MATRIX.
   80 IT = 0
      ...
      DO 85 I=2,NR1
         IJ = NR1-I+2
         CALL VDCMPA(KC,A(IJ-1,JCOLA),INKA,A(IJ,JCOLA),INKA,KOMPAR)
   ...
   90 IR(1) = -IR(1)
 9000 CONTINUE
      IF (IER.NE.0) CALL UERTST(IER,6HVSIDUQ)
 9005 RETURN
      END
