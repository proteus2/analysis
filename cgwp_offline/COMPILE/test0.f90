...
SUBROUTINE input_prof
  ...
  real, dimension(27) ::  z_ei, u_ei, t_ei
  real, dimension(1,27,5) ::  u_ei0, t_ei0

  fdir = '..'

  call opennc(trim(fdir)//'/era.u.ml.20051101.nc',ncid)
  call get3d(ncid,'u',1,27,5,u_ei0)
  call closenc(ncid)
  call opennc(trim(fdir)//'/era.t.ml.20051101.nc',ncid)
  call get3d(ncid,'t',1,27,5,t_ei0)
  call closenc(ncid)

  u_ei(:) = 0.125*(u_ei0(1,:,1) + u_ei0(1,:,2)) + 0.25*sum(u_ei0(1,:,3:5), dim=2)
  t_ei(:) = 0.125*(t_ei0(1,:,1) + t_ei0(1,:,2)) + 0.25*sum(t_ei0(1,:,3:5), dim=2)

  z_ei(:) = 7.e3*log(1.e3 /  &
(/0.1, 0.282171, 0.499803, 0.785599, 1.13959, 1.56403, 2.06498, 2.65364, &
3.34817, 4.17657, 5.18153, 6.4198, 7.95398, 9.85477, 12.2098, 15.1277, 18.7429,&
23.222, 28.7715, 35.6473, 44.1661, 54.4347, 66.415, 80.173, 95.7377, 113.158, &
132.454, 153.624, 176.64, 201.456, 228.005, 256.262, 286.229, 317.851, 351.023,&
385.594, 421.377, 458.148, 495.657, 533.629, 571.773, 609.787, 647.361, &
684.184, 719.95, 754.364, 787.146, 818.034, 846.795, 873.229, 897.17, 918.498, &
937.138, 953.071, 966.336, 977.036, 985.344, 991.509, 995.86, 998.815/)  &
 )
