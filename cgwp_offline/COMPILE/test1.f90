...
!p_coord+
  do k=2, nz_b-1
    nbv_fl_b(:,k) = (g*g)/t_fl_b(:,k)*( 1./cp + 7.e3/(rd*t_fl_b(:,k))*   &
        ( (t_fl_b(:,k+1) - t_fl_b(:,k-1))/  &
          (zp_flev_ref(k+1) - zp_flev_ref(k-1)) ) )
  enddo
!p_coord-
  nbv_fl_b(:,1   ) = nbv_fl_b(:,2     )
  ...
  do k=1, nz_b
    rho_fl_b(:,k) = p_flev_ref(k)/(rd*t_fl_b(:,k))
  enddo
...
!  ndate = 30  ;  date = 1  ;  hour = hh(1)  ! for get_ifilename
  ...
  allocate( p_dlev_ref(nz_b), zp_dlev_ref(nz_b), zp_flev_ref(nz_b),      &
            lnp_flev_ref(nz_b), p_flev_ref(nz_b) )
  ...
  zp_dlev_ref(2:nz_b) = 0.5*(zp_flev_ref(1:nz_b-1) +                     &
                             zp_flev_ref(2:nz_b  ))
  zp_dlev_ref(1) = 2.*zp_flev_ref(1) - zp_dlev_ref(2)
  p_dlev_ref(:) = 1.e5*exp(-zp_dlev_ref(:)/7.e3)
  lnp_flev_ref(:) = log(p_flev_ref(:))
...
SUBROUTINE put_vars_set
  ...
  ndim2d_put = (/nz_b-1,ncol/)
  zo(:) = zp_dlev_ref(:)
  if ( l_drag_u_o ) then
    call putset(iv,'drag_u',transpose(drag_u(:,2:)), zo(2:),coln)
  end if
  if ( l_drag_v_o ) then
    call putset(iv,'drag_v',transpose(drag_v(:,2:)), zo(2:),coln)
  end if
...
SUBROUTINE put_vars_zm_set
  ...
  ndim2d_put = (/ny2,nz_b-1/)
  zo(:) = zp_dlev_ref(:)
  if ( l_drag_u_o ) then
    call putset(iv,'drag_u',zm_drag_u(:,2:), lato,zo(2:))
  end if
  if ( l_drag_v_o ) then
    call putset(iv,'drag_v',zm_drag_v(:,2:), lato,zo(2:))
  end if
