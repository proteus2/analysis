...
!---------------------------------------------------------------
   ...
   type ::  vset6
     real, dimension(:,:,:,:,:,:), allocatable ::  var_out
     real, dimension(:)          , allocatable ::  axis1, axis2, axis3,  &
                                                   axis4, axis5, axis6
     integer                                   ::  nd(6)
     character(len=32)                         ::  vname, axis(6)
   end type vset6

   type ::  vsetd
     integer          , dimension(:), allocatable ::  nd
     character(len=32), dimension(:), allocatable ::  axis
...
SUBROUTINE outnc_def(ncid,nvar,setd,title,maxd,varid,axid,tag,nmode)

  implicit none

  integer,                      intent(in) ::  ncid, nvar, maxd
  type(vsetd), dimension(nvar), intent(in) ::  setd
  character(len=*),             intent(in) ::  title
  integer, optional,            intent(in) ::  nmode

  integer, dimension(nvar)     , intent(out) ::  varid
  integer, dimension(maxd,nvar), intent(out) ::  axid, tag

  integer ::  iv, ia, cnt, ii, st, ndim
  integer ::  n_rec
  integer, dimension(nvar*maxd) ::  dimid
  integer, dimension(maxd)      ::  vardimid
  character(len=64)             ::  ctemp(nvar*maxd)

  n_rec = 0
  if ( present(nmode) ) then
    if ( nmode >= 1 .and. nmode <= maxd )  n_rec = nmode
  end if
  ...
  do iv=1, nvar
    ...
    do ia=1, maxd
      cnt = (iv-1)*maxd + ia
      ctemp(cnt) = setd(iv)%axis(ia)
      if ( setd(iv)%nd(ia) < 1 .or. trim(ctemp(cnt)) == '' ) then
        tag(ia,iv) = 0
      else
        ndim = ndim + 1
        tag(ia,iv) = cnt
        ...
        if (tag(ia,iv) == cnt) then
          if (ia == n_rec) then
            ...
            st = nf_def_dim(ncid,trim(ctemp(cnt)),setd(iv)%nd(ia),dimid(cnt))
          end if
          st = nf_def_var(ncid,trim(ctemp(cnt)),nf_real,1,dimid(cnt),axid(ia,iv))
        end if
        vardimid(ndim) = dimid(tag(ia,iv))
      end if
    enddo
    st = nf_def_var(ncid,trim(setd(iv)%vname),nf_real,ndim,vardimid(1:ndim),varid(iv))
...
SUBROUTINE outnc(wname,nvar,set,title,nmode)
  ...
  integer, parameter ::  maxd = 4

  integer ::  iv, ia, ncid, cnt, st
  integer, dimension(maxd,nvar) ::  axid, tag
  integer, dimension(nvar)      ::  varid
  type(vsetd), dimension(nvar)  ::  setd

  do iv=1, nvar
    allocate( setd(iv)%axis(maxd), setd(iv)%nd(maxd) )
    setd(iv)%vname   = set(iv)%vname
    setd(iv)%axis(:) = set(iv)%axis(:)
    setd(iv)%nd  (:) = set(iv)%nd  (:)
  enddo

  st = nf_create(trim(wname),nf_share,ncid)

  call outnc_def(ncid,nvar,setd,title,maxd, varid,axid,tag, nmode)

  do iv=1, nvar
    cnt = (iv-1)*maxd
    if ( tag(1,iv) == (cnt + 1) )    &
       st = nf_put_var_real(ncid,axid(1,iv),set(iv)%axis1)
    if ( tag(2,iv) == (cnt + 2) )    &
       st = nf_put_var_real(ncid,axid(2,iv),set(iv)%axis2)
    if ( tag(3,iv) == (cnt + 3) )    &
       st = nf_put_var_real(ncid,axid(3,iv),set(iv)%axis3)
    if ( tag(4,iv) == (cnt + 4) )    &
       st = nf_put_var_real(ncid,axid(4,iv),set(iv)%axis4)
  enddo
...
SUBROUTINE outnc_blank(wname,nvar,set0,title,nmode)
  ...
  do iv=1, nvar
    allocate( setd(iv)%axis(maxd), setd(iv)%nd(maxd) )
    setd(iv)%vname   = set0(iv)%vname
    setd(iv)%axis(:) = set0(iv)%axis(:)
    setd(iv)%nd  (:) = set0(iv)%nd  (:)
  enddo
  ...
  do iv=1, nvar
    cnt = (iv-1)*maxd
    if ( tag(1,iv) == (cnt + 1) )    &
       st = nf_put_var_real(ncid,axid(1,iv),set0(iv)%axis1)
    if ( tag(2,iv) == (cnt + 2) )    &
       st = nf_put_var_real(ncid,axid(2,iv),set0(iv)%axis2)
    if ( tag(3,iv) == (cnt + 3) )    &
       st = nf_put_var_real(ncid,axid(3,iv),set0(iv)%axis3)
    if ( tag(4,iv) == (cnt + 4) )    &
       st = nf_put_var_real(ncid,axid(4,iv),set0(iv)%axis4)
  enddo
...
SUBROUTINE outnc6d(wname,nvar,set,title,nmode)

  implicit none

  integer,                      intent(in) ::  nvar
  type(vset6), dimension(nvar), intent(in) ::  set
  character(len=*),             intent(in) ::  wname, title
  integer, optional,            intent(in) ::  nmode

  integer, parameter ::  maxd = 6
  ...
  do iv=1, nvar
    ...
    if ( tag(4,iv) == (cnt + 4) )    &
       st = nf_put_var_real(ncid,axid(4,iv),set(iv)%axis4)
    if ( tag(5,iv) == (cnt + 5) )    &
       st = nf_put_var_real(ncid,axid(5,iv),set(iv)%axis5)
    if ( tag(6,iv) == (cnt + 6) )    &
       st = nf_put_var_real(ncid,axid(6,iv),set(iv)%axis6)
  enddo
