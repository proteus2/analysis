...
P995=( s FILE_I_HEAD2  '' )
P996=( s FILE_I_FORM2  ${P101[2]}.fft_XXXX_koi.XXXX.XXXX.nc  -999 )
...
M=$M1        ;  while [ $M    -le $M2    ] ; do
  ...
  P102[2]=$YYYY
  P103[2]=$MM
  IDIR2=/data4/kyh/dat/L60CGW/fftkcoef/$YYYY
  P995[2]="$IDIR2"
  ODIR=/data4/kyh/dat/L60CGW/epf/$YYYY
#/data17/kyh/dat/NIMR/epf/$YYYY
  P999[2]="$ODIR/${P101[2]}.epf_koiyz.$YYYY.${MM}.nc"
  P1000[2]="$ODIR/${P101[2]}.epf2_koiyz.$YYYY.${MM}.nc"
  if [ ! -d $ODIR ] ; then mkdir -p $ODIR ; fi
#  [ -e ${P999[2]} ] && mv ${P999[2]} $ODIR/old.${P101[2]}.epf_koiyz.$YYYY.${MM}.nc
#  [ -e ${P1000[2]} ] && mv ${P1000[2]} $ODIR/old.${P101[2]}.epf2_koiyz.$YYYY.${MM}.nc

  # create namelist --------------------
  cr_file $F_NAMELIST
  cr_nl $F_NAMELIST ANALCASE "${P101[*]}" "${P102[*]}" "${P103[*]}" "${P104[*]}" "${P105[*]}" "${P106[*]}"
  cr_nl $F_NAMELIST PARAM "${P201[*]}" "${P202[*]}" "${P203[*]}" "${P204[*]}" "${P205[*]}"
  cr_nl $F_NAMELIST FILEIO "${P901[*]}" "${P902[*]}" "${P990[*]}" "${P991[*]}" "${P992[*]}" "${P993[*]}" "${P994[*]}" "${P995[*]}" "${P996[*]}" "${P997[*]}" "${P998[*]}" "${P999[*]}" "${P1000[*]}"
  cat $F_NAMELIST
