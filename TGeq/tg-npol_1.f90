PROGRAM Taylor_Goldstein_eqn
  ...
  use T_G_eqn
  use nr, only: spline, splint, lfit
  use netio
...
!  parameter  ( r_coef = (0.,0.) )
  ...
  real,    dimension(nabs,nphi,nz) ::  wr3, wi3, r3, f3, f_pm3, f_nm3
  real,    dimension(nabs,nphi,nz) ::  wr_pm3, wi_pm3, wr_nm3, wi_nm3
  ...
  integer ::  ncid, nzi
  real    ::  phi, cp, hrho, tmp, a(nb), covar(nb,nb), arr(nb)
  logical ::  lerr, maska(nb)
...
maska(:) = .true.
a    (:) = 0.
call lfit(zi,nb2i,tmpcs,a,maska,covar,tmp,funcs)
do k=1, nzi
  call funcs(zi(k),arr)
  nb2i(k) = sum(a(:)*arr(:))
enddo
...
! dump
!
  call out3d1d('res3.nc',10,(/'wr    ','wi','r', &
               'wr_pm','wi_pm','wr_nm','wi_nm','f','f_pm','f_nm'/), &
               (/wr3,wi3,r3,wr_pm3,wi_pm3,wr_nm3,wi_nm3,f3,f_pm3,f_nm3/), &
               'R_abs',nabs,r_coef_abs,'R_phi',nphi,r_coef_phi, &
               'z3',nz,z/1.e3, &
               1,(/'m2'/),m2,'z',nz,z/1.e3,'T-G eqn solution')
