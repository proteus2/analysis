...
P205=( n NMON_PATCH  1 )
#P206=( n K_SUM  -999 )   # EPF for each k <= K_SUM and EPF sum for k > K_SUM
#P207=( n LAT_SMPL0  -999 )   # base lat. for subsampling after 1-2-1 averaging in latitudinal direction
P206=( n K_SUM  $(( $RA_NX / 4 )) )
P207=( n LAT_SMPL0  0.0 )
#== Parameter 9 - I/O ============================
P901=( n NT_F4  ${NT_F4[@]} )
...
P997=( s FILE_I_FORM2  XXXX/${P101[2]}.fft_XXXX_ko.XXXX.XXXX.nc  -999 )
...
M=$M1        ;  while [ $M    -le $M2    ] ; do
  ...
  cr_nl $F_NAMELIST FILEIO "${P901[*]}" "${P902[*]}" "${P991[*]}" "${P992[*]}" "${P993[*]}" "${P994[*]}" "${P995[*]}" "${P996[*]}" "${P997[*]}" "${P998[*]}" "${P981[*]}" "${P982[*]}" "${P999[*]}" "${P1000[*]}"
  cat $F_NAMELIST
