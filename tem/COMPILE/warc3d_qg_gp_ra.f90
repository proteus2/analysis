...
! attributes (scale facter, add_offset, "_FillValue" or "missing_value") must be considered.
  ...
  real, dimension(:,:),     allocatable ::  pt0, u0, v0, w0
...
if (date == 3)  exit  !yh
  ...
  ! get variable
  allocate( v(nx,ny2,nz2), w(nx,ny2,nz2) )
  call get_4var
...
enddo
  ...
  w0 (:,:) = sum(w ,dim=1)/float(nx)
  pt0(:,:) = sum(pt,dim=1)/float(nx)

  w0(:,:) = -w0(:,:)*spread(h_scale/(p0a(:)*100.),1,ny2)

  deallocate( v, w )

  ! calculate zonal mean
  call warc_s_qg_gp(                                                     &
      nx,ny2,nz2,lat0,p0a*100.,gp,pt0,dlon,h_scale,1.e32,                &
...
! END
  ...
  allocate( gp(nx,ny2,nz2), u(nx,ny2,nz2), pt(nx,ny2,nz2) )
  allocate( pt0(ny2,nz2), u0(ny2,nz2), v0(ny2,nz2), w0(ny2,nz2) )
