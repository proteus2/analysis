...
enddo
  ...
  if (i_time == i_time_last) then
    ...
    if (i_time /= 1) then
      var4d(:,:,i_time_last,:) = var4d(:,:,i_time_last,:) +              &
                                 var3d(:,:,:)/(ntavg*2.)
    end if
    if (imon /= nmon+1) then
      var4d(:,:,i_time,:) = var4d(:,:,i_time,:) + var3d(:,:,:)/(ntavg*2.)
    end if
  end if
