...
! m_avg must be completed.
  ...
  real,    dimension(:,:,:),   allocatable ::  u, v, pt, w
...
! READ NAMELISTS
  ...
  if (.not. allocated(lon)) then
    call getdim(file_i(3),var_i_name(3))  ! temperature
    ovarname(1:10) = varname_tem_qg(1:10)
    ovarname(11) = 'u_tend'
    ovarname(12) = 't_tend'
    if (opt_avrg == 0) then
  ...
  ! get variable
  allocate( u(nx,ny,nz), v(nx,ny,nz), pt(nx,ny,nz), wm(ny,nz) )
  allocate( w(nx,ny,nz) )
...
enddo
  ...
  call tem_qg(                                                           &
      nx,ny,nz,lat,p*100.,u,v,pt,wm,h_scale,1.e32,                       &
  ...
  if (i_time == i_time_last) then
    var4d(:,:,i_time,:10) = var4d(:,:,i_time,:10) +                      &
                            var3d(:,:,:10)/ntavg
  else
    var3d(:,:,11) = sum(u , dim=1)/float(nx)
    var3d(:,:,12) = sum(pt, dim=1)/float(nx)
    var3d(:,:,12) = var3d(:,:,12)*spread((p(:)*100./p0)**kappa,1,ny)
    if (opt_avrg == 0)  var3d(:,:,11:12) = var3d(:,:,11:12)*float(nhour)

    if (i_time /= 1) then
      var4d(:,:,i_time_last,:10) = var4d(:,:,i_time_last,:10) +          &
                                   var3d(:,:,:10)/(ntavg*2.)
      var4d(:,:,i_time_last,11:12) = var4d(:,:,i_time_last,11:12) +      &
                                  var3d(:,:,11:12)
    end if
    if (imon /= nmon+1) then
      var4d(:,:,i_time,:10) = var4d(:,:,i_time,:10) +                    &
                              var3d(:,:,:10)/(ntavg*2.)
      var4d(:,:,i_time,11:12) = var4d(:,:,i_time,11:12) - var3d(:,:,11:12)
    end if
  end if

  deallocate( l_tag2d )
  deallocate( u, v, pt, wm )
