...
! KW(3)
  ...
  where ( sum(abs(var4d(:,o1r:-1,:,1:8)), dim=4) /= 0. )
    var6d(:,o1r:-1,:,k,1,4) = 0.
    var6d(:,o1r:-1,:,k,2,4) = 0.
    var6d(:,o1r:-1,:,k,3,4) = 0.
    var6d(:,o1r:-1,:,k,4,4) = 0.
...
!  where ( sum(abs(var4d(:,o1r:o2r,:,9:16)), dim=4) /= 0. )
  where ( any(var4d(:,o1r:o2r,:,9:16) /= 0., dim=4) )
    var6d(:,o1r:o2r,:,k,1,4) = 0.
    var6d(:,o1r:o2r,:,k,2,4) = 0.
    var6d(:,o1r:o2r,:,k,3,4) = 0.
    var6d(:,o1r:o2r,:,k,4,4) = 0.
    var6d(:,o1r:o2r,:,k,5,4) = 0.
    var6d(:,o1r:o2r,:,k,6,4) = 0.
    var6d(:,o1r:o2r,:,k,7,4) = 0.
    var6d(:,o1r:o2r,:,k,8,4) = 0.
  end where
  ...
  var6d(:,o1m:o1r-1,:,k,:,5) = var4d(:,o1m:o1r-1,:,9:16)
  var6d(:,o2r+1:o2m,:,k,:,6) = var4d(:,o2r+1:o2m,:,9:16)

  ! IGW(7,8) step 3
  varo(:,k,:,7) = varo(:,k,:,7) - tmp1(:,:)
  varo(:,k,:,8) = varo(:,k,:,8) - tmp2(:,:)

!  where ( sum(abs(var4d(:,o1m:o1r-1,:,9:16)), dim=4) /= 0. )
  where ( any(var4d(:,o1m:o1r-1,:,9:16) /= 0., dim=4) )
    var6d(:,o1m:o1r-1,:,k,1,7) = 0.
    var6d(:,o1m:o1r-1,:,k,2,7) = 0.
    var6d(:,o1m:o1r-1,:,k,3,7) = 0.
    var6d(:,o1m:o1r-1,:,k,4,7) = 0.
    var6d(:,o1m:o1r-1,:,k,5,7) = 0.
    var6d(:,o1m:o1r-1,:,k,6,7) = 0.
    var6d(:,o1m:o1r-1,:,k,7,7) = 0.
    var6d(:,o1m:o1r-1,:,k,8,7) = 0.
  end where
!  where ( sum(abs(var4d(:,o2r+1:o2m,:,9:16)), dim=4) /= 0. )
  where ( any(var4d(:,o2r+1:o2m,:,9:16) /= 0., dim=4) )
    var6d(:,o2r+1:o2m,:,k,1,8) = 0.
    var6d(:,o2r+1:o2m,:,k,2,8) = 0.
    var6d(:,o2r+1:o2m,:,k,3,8) = 0.
    var6d(:,o2r+1:o2m,:,k,4,8) = 0.
    var6d(:,o2r+1:o2m,:,k,5,8) = 0.
    var6d(:,o2r+1:o2m,:,k,6,8) = 0.
    var6d(:,o2r+1:o2m,:,k,7,8) = 0.
    var6d(:,o2r+1:o2m,:,k,8,8) = 0.
  end where
...
! temporary : (S+A) + symm-antisymm cross-correl.
  ...
  var6d(:,:-1,:,k,:,9 ) = var6d(:,:-1,:,k,:,1) + var6d(:,:-1,:,k,:,11)
  var6d(:,0: ,:,k,:,10) = var6d(:,0: ,:,k,:,2) + var6d(:,0: ,:,k,:,12)

  var6d(:,:-1,:,k,:,19) = var6d(:,:-1,:,k,:,9 )
  var6d(:,0: ,:,k,:,19) = var6d(:,0: ,:,k,:,10)

  var6d(:,:,:,k,:,20) = var6d(:,:,:,k,:,4) + var6d(:,:,:,k,:,14)

  var6d(:,:-1,:,k,:,21) = var6d(:,:-1,:,k,:,7) + var6d(:,:-1,:,k,:,17)
  var6d(:,0: ,:,k,:,22) = var6d(:,0: ,:,k,:,8) + var6d(:,0: ,:,k,:,18)

!yh+
if (1 == 1) then
varo(:,:,:,:) = sum(sum(var6d(:,:,:,:,:,:), dim=1), dim=1)
end if
!yh-
