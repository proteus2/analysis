...
! GET AXES AND INITIALIZE ARRAYS
  ...
  print*, ' Calculating...'
  call epf_z_cp_fc2(                                                              &
     nk,nome*nwavgrp,ny2,nz2,lat0,ht0,ht_th0,nt,um(:,:,1:nt),ptm(:,:,1:nt),       &
     rhom(:,:,1:nt),1.e32,                                                        &
...
! END
  ...
  print*, trim(file_i(2))
  iv_i = 2  ;  var3d(:,:,1:nz2) = get_ivara3d(1,nx,iy2(1),ny2,iz2(1),nz2)
  ptm(:,:,i_time) = sum(var3d(:,:,1:nz2), dim=1)/float(nx)
  print*, trim(file_i(3))
