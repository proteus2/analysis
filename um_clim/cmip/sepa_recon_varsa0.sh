...
P202=( n K_LARGEST  -999 )
P203=( n PERIOD_SMALLEST  -999 )
P204=( n LAT_RNG  0.0  30.0 )
...
P990=( s FID  pj )
P991=( s FILE_I_HEAD  "/data4/kyh/dat/L60CGW/fftkcoef" )
P992=( s FILE_I_FORM  XXXX/${P101[2]}.fft_${P201[2]}_wave_ko.XXXX.XXXX.nc  -999 )
P993=( s FILE_I_XXXX  YYYY                                   YYYY  MM      -999 )
P994=( s VAR_I_NAME  fcr_${P201[2]}_s  fci_${P201[2]}_a \
                     fcr_${P201[2]}_s  fci_${P201[2]}_a )  # Do not change this order.
P995=( s FILE_I_HEAD2  "/data15/HG2AMIP/L60CGW" )
...
M=$M1        ;  while [ $M    -le $M2    ] ; do
  ...
  P102[2]=$YYYY
  P103[2]=$MM
  ODIR=/data4/kyh/dat/L60CGW/reconstr/$YYYY
  P999[2]="$ODIR/${P101[2]}.var_wave_yz_recon0.$YYYY.${MM}.nc"
...
#  [ -e ${P999[2]} ] && mv ${P999[2]} $ODIR/old.${P101[2]}.epfsa0_yz_recon0.$YYYY.${MM}.nc
  ...
  cr_nl $F_NAMELIST FILEIO "${P901[*]}" "${P902[*]}" "${P990[*]}" "${P991[*]}" "${P992[*]}" "${P993[*]}" "${P994[*]}" "${P995[*]}" "${P996[*]}" "${P997[*]}" "${P998[*]}" "${P999[*]}"
  cat $F_NAMELIST
