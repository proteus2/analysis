...
P994=( s VAR_I_NAME  dch_max  vars_cgw  u  v )
#P990=( s FID  pb )
...
M=$M1        ;  while [ $M    -le $M2    ] ; do
  ...
  P999[2]="$ODIR/${P101[2]}.dchm_pdf.$YYYY.${MM}.nc"
  if [ ! -d $ODIR ] ; then mkdir -p $ODIR ; fi
#  [ -e ${P999[2]} ] && mv ${P999[2]} $ODIR/old.${P101[2]}.dchm_pdf.$YYYY.${MM}.nc

  # create namelist --------------------
  cr_file $F_NAMELIST
  cr_nl $F_NAMELIST ANALCASE "${P101[*]}" "${P102[*]}" "${P103[*]}" "${P104[*]}" "${P105[*]}" "${P106[*]}"
  cr_nl $F_NAMELIST PARAM "${P201[*]}" "${P202[*]}"
  cr_nl $F_NAMELIST FILEIO "${P901[*]}" "${P902[*]}" "${P903[*]}" "${P990[*]}" "${P991[*]}" "${P992[*]}" "${P993[*]}" "${P994[*]}" "${P996[*]}" "${P997[*]}" "${P999[*]}"
  cat $F_NAMELIST
