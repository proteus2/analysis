...
! random_order
! Initialize (seed) the uniform random number generator for ANY
! compiler
! seed_random_number
...
! The compilers own random number generator, SUBROUTINE
! RANDOM_NUMBER(r),
! is used to provide a source of uniformly distributed random numbers.
 
! N.B. At this stage, only one random number is generated at each call
! to
...
!
! In most of Dagpunar's routines, there is a test to see whether the
! value
! of one or two floating-point parameters has changed since the last
! call.
...
! parameters, and .FALSE. if the parameter values are the same as for
! the
! previous call.
 
! Version 1.10, 1 August 1996
! Changes from version 1.01
! 1. The random_order, random_Poisson & random_binomial routines have
! been
! replaced with more efficient routines.
! 2. A routine, seed_random_number, has been added to seed the uniform
! random
! number generator. This requires input of the required number of
! seeds
! for the particular compiler from a specified I/O unit such as a
! keyboard.
...
! and J.FALSE. Monahan augmented with quadratic bounding curves.
...
! Generate P = (u,v) uniform in rectangle enclosing acceptance
...
! OR IF THE DISTRIBUTION HAS CHANGED SINCE THE LAST CALL OF THE
...
! Generates a single random deviate from a Poisson distribution with
...
! STEP N. NORMAL SAMPLE - random_normal() FOR STANDARD NORMAL
...
! STEP E. EXPONENTIAL SAMPLE - random_exponential() FOR STANDARD
...
! FIRST = .TRUE. for the first call using the current parameter
...
! N.B. The real Stirling's formula is used here, not the simpler, but

! less

! accurate formula given by De Moivre in a letter to Stirling,
...
! FIRST --> Set FIRST = .TRUE. for the first call to perform

! initialization

! the set FIRST = .FALSE. for further calls using the same
...
! random_binomial2 <-- A random deviate yielding the number of
...
! THE PARAMETER H IS SET SO THAT UNSTORED INVERSION ONLY IS USED WHEN P
