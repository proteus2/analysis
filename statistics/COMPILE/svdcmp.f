...
c
   ...
   39 continue
c
c     diagnoalization of the bidiagonal form
c
      do 49 k=n,1,-1
c         ! loop over singular value
        do 48 its=1,30
c         ! loop over allowed iterations
          do 41 l=k,1,-1
c         ! test for splitting 
            nm=l-1
c         ! note that rv(1) is always zero
            if ((abs(rv1(l))+anorm).eq.anorm) go to 2
            if ((abs(w(nm))+anorm).eq.anorm) go to 1
   41     continue
    1     c=0.
c         ! cancellation of rv1(l), if l.gt 1
          s=1.0
          do 43 i=l,k
            f=s*rv1(i)
            rv1(i)=c*rv1(i)
