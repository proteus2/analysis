program ko_spectrum_S
  ...
  real,    parameter :: ptbar1 = 300.2588  !240.5123  !300.2588

  integer, parameter :: nst =  61,    nnd = 360
  real,    parameter :: xst =   0.e3, xnd =1200.e3
  real,    parameter :: zst =  65.e3, znd =  85.e3, zint = 20.e3
...
print*,ubar
  ...
  do i=2, nkk
    mch2(i,n,k) = n2bar(k) - (ubar(k)-cc(i,n))*(d2udz(k)+dudz(k)/h_scale) &
                 - (ubar(k)-cc(i,n))**2*((2.*3.141593*kk(i))**2+0.25/h_scale**2)
  enddo
  enddo
  enddo
!!
...
!!
  ...
  do n=1, noo
  do i=2, nkk
    ss(i,n,k) = ss(i,n,k) * (0.,1.)*2.*3.141593*kk(i)*(ubar(k)-cc(i,n))
  enddo
  enddo
  enddo
...
!      if (abs(mch2(i,n,k)) .le. 1.e-4)  tmpi = 1
!    enddo
!    do k=kst(kre), knd(kre)
!      if (tmpi .eq. 0) then
!        s_tot(i,n,k) = s_tot(i,n,k) / mch2(i,n,k)
!      else
!        s_tot(i,n,k) = 0.
!      end if
!    enddo
...
!  enddo
  ...
  do i=1, nkk
    ...
    enddo
    d_ave(i,n,kre) = d_ave(i,n,kre) * dx*dt/nxp/nt
    if (d_ave(i,n,kre) .eq. 0.)  d_ave(i,n,kre) = 1.e32
  enddo
  enddo
  enddo
