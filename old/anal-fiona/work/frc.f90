...
!****************** hyun *************************
  integer, parameter :: nx =1402 , nxn =1202, xst = 100
  integer, parameter :: nz = 472 , nzn = 252, zst = 100  !100
  integer, parameter :: dt =3600   ! [s]
  integer, parameter :: init =     0
  integer, parameter :: fint =  7200
  integer, parameter :: nt = (fint - init) / dt
  integer, parameter :: fwopt = 1, fuopt = 0, meanbase = 0
  integer, parameter :: binopt = 0, ncopt = 2, mixopt = 1
...
!***************************************************
  ...
  open(10,file='/usr/users/kyh/arps/work/2ndgen/x12jul/rhobar')
  do k=1, nz
    read(10,*) rhodata(k)
  enddo
  rhobar(:) = rhodata(zst+1:zst+nzn)
  open(20,file='/export10/kyh/ing/105jul/bin/ubar.bin', &
  ...
  do n=1, nt
    ...
    open(1,file='/export10/kyh/ing/105jul/bin/uprt.bin'&
     //timsnd(1:tmstrln),form='unformatted',convert='big_endian')
    read(1) prt 
    close(1) 
    uprt(:,:) = prt(xst+1:xst+nxn,zst+1:zst+nzn)
    open(2,file='/export10/kyh/ing/105jul/bin/w.bin'&
     //timsnd(1:tmstrln),form='unformatted',convert='big_endian')
    read(2) prt
    close(2)
    wprt(:,:) = prt(xst+1:xst+nxn,zst+1:zst+nzn)
    if (mixopt .eq. 1) then
      open(3,file='/export10/kyh/ing/105jul/bin/wtmix.bin'&
       //timsnd(1:tmstrln),form='unformatted',convert='big_endian')
      read(3) prt
      close(3)
      wtmix(:,:) = prt(xst+1:xst+nxn,zst+1:zst+nzn)
      open(4,file='/export10/kyh/ing/105jul/bin/wmix.bin'&
       //timsnd(1:tmstrln),form='unformatted',convert='big_endian')
      read(4) prt
      close(4)
    ...
    ! write
    if (binopt .eq. 1) then
      write(fname,'(a,i6.6)') '/export10/kyh/ing/105jul/fw/fw1.bin',time
      open(101,file=trim(fname), form='unformatted', convert='big_endian')
      write(101) fw1
      close(101)
      write(fname,'(a,i6.6)') '/export10/kyh/ing/105jul/fw/fw2.bin',time
      open(102,file=trim(fname), form='unformatted', convert='big_endian')
      write(102) fw2
      close(102)
      if (mixopt .eq. 1) then
        write(fname,'(a,i6.6)') '/export10/kyh/ing/105jul/fw/fwtmix.bin',time
        open(103,file=trim(fname), form='unformatted', convert='big_endian')
        write(103) wtmix
        close(103)
        write(fname,'(a,i6.6)') '/export10/kyh/ing/105jul/fw/fwmix.bin',time
    ...
    if (ncopt .eq. 1) then
      ...
      write(fname,'(a,i5.5,a)') '/export22/kyh/ing/105jul/fwnc/fw',time,'.nc'
      if (mixopt .eq. 0) then
