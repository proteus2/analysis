program uw_test              ! check units
 ...
 integer, parameter :: nxp =1200, nzp = 350, nt = 480
...
print*, 'data reading'
 do n=1, nt
   ...
   open(1,file='/export23/kyh/105jul/bin/uprt.bin'//time,    &
          form='unformatted',convert='big_endian')
   read(1) udata
   close(1)
   open(2,file='/export23/kyh/105jul/bin/w.bin'//time,       &
          form='unformatted',convert='big_endian')
   read(2) wdata
   close(2)
 ...
 if (pwanal .ne. 0) then
   do n=1, nt
     write(time,'(i6.6)') int(t(n))
     open(3,file='/export23/kyh/105jul/bin/pprt.bin'//time,    &
            form='unformatted',convert='big_endian')
     read(3) pdata
     close(3)
