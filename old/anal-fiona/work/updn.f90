...
!-----------------------------------------------------
 ...
 integer, parameter :: fieldopt = 0, coopt = 1, psopt = 0, norm = 1, winopt = 0
 integer, parameter :: nx =1002, nz = 422, ntall = 240
 integer, parameter :: nxp = 800, nzp = 300, nt = 180
 integer, parameter :: xst = 100, zst =  0
 real,    parameter :: dx = 1000., dt = 60., t0 = 3600, dz = 300.
!-----------------------------------------------------
 integer, parameter :: nkknew = nxp/2+1, noonew = nt/2*2+1
 integer :: i,j,n, ncid, ii, nn, jj1, jj2
 real    :: x(nxp), t(nt), z(nzp), kk(nxp), oo(nt), dzr, an, bn
 real    :: ccint, mabs, mabsl, mabsu, co, pi, m2upper, m2lower, delta, termwkb
 real    :: kknew(nkknew), oonew(noonew), rhort(nz), m2(nkknew,noonew,nzp), window(nt)
 real*8  :: rhobar(nz)
 ...
 open(4,file='/usr/users/kyh/anal/work/rhoallid')
 read(4,*) rhort
 close(4)
 open(5,file='/usr/users/kyh/ideal/work/id0/rhobar')
 read(5,*) rhobar
 close(5)
 if (norm .eq. 0) then
   rhort = 1.
...
! read ubar or umean
 call opennc('/usr/users/kyh/result4/id0/UNbar/unbar.nc',ncid)
...
! read m^2
 call opennc('/export22/kyh/ing/id0/spec/msq/m2.nc',ncid)
...
! window ftn
 ...
 bn = (nt + 1.) / 2.
 do n=1, nt
   window(n) = 1. - ((n-an)/bn)**2
 enddo
 if (winopt .eq. 0)  window(:) = 1.

! read u, w
print*, 'data reading'
 do n=1, nt
   write(time,'(i6.6)') int(t(n))
   open(1,file='/export22/kyh/ing/id0/bin/uprt.bin'//time,    &
          form='unformatted',convert='big_endian')
   read(1) udata
   close(1)
   open(2,file='/export22/kyh/ing/id0/bin/w.bin'//time,       &
          form='unformatted',convert='big_endian')
   read(2) wdata
   close(2)
   ...
   do j=1, nzp
   do i=1, nxp
     ua(i,j,n) = udata(xst+1+i,zst+1+j) * rhort(zst+1+j) * window(n)
     wa(i,j,n) = wdata(xst+1+i,zst+1+j) * rhort(zst+1+j) * window(n)
   enddo
   enddo
 enddo
print*, 'OK...'
 ...
 do j=1, nzp
   ...
   if (j .eq. nzp)  jj2 = nzp
   dzr = dz
   if (j.eq.1 .or. j.eq.nzp)  dzr = dz / 2.
...
! 2-D FFT & determine up/downward
   ...
   do i=2, nxp
     ...
     ! up/dn
     if ((m2(ii,nn,j).gt.m2lower .and. m2(ii,nn,j).le.m2upper) .and. &
         (m2(ii,nn,jj1).gt.0. .and. m2(ii,nn,jj2).gt.0.)) then

       mabs  = 2.*pi * sqrt(m2(ii,nn,j))
       mabsl = 2.*pi * sqrt(m2(ii,nn,jj1))
       mabsu = 2.*pi * sqrt(m2(ii,nn,jj2))

       delta = abs( (mabsu-2.*mabs+mabsl) / dzr**2 / 2. - &
                 ((mabsu-mabsl) / (2.*dzr))**2 * 0.75/mabs ) &
                / mabs**3
! delta approximated.
!       delta = abs(mabsu-mabsl) / (2.*dzr) / mabs**2

if (delta .lt. 1.0) then

       co = real(real(cu(i,n))*real(cw(i,n))+aimag(cu(i,n))*aimag(cw(i,n)))

       termwkb = 0.5 / mabs * (mabsu-mabsl) / (2.*dzr)
!termwkb = 0.
       dd = (cu(i,n)*kk(i) + iii*(hinv(j)+termwkb)*cw(i,n)) / sign(mabs,kk(i))

       cw_pos = (cw(i,n) + dd) / 2.
       cw_neg = (cw(i,n) - dd) / 2.

       cu_pos = cw_pos * (sign(mabs,kk(i)) - iii*(hinv(j)+termwkb)) / kk(i)
       cu_neg = cw_neg * (-sign(mabs,kk(i)) - iii*(hinv(j)+termwkb)) / kk(i)
...
! output
 if (fieldopt .eq. 1) then
   write(fname,'(a)') '/export24/kyh/wup1a.nc'
   call out3d(trim(fname),1,(/'w_up'/),wa_up,                          &
               'x',nxp,x/1000.,'z',nzp,z/1000.,'t',nt,t/60.,           &
               'w upward')
   write(fname,'(a)') '/export24/kyh/wdn1a.nc'
   call out3d(trim(fname),1,(/'w_dn'/),wa_dn,                          &
               'x',nxp,x/1000.,'z',nzp,z/1000.,'t',nt,t/60.,           &
               'w downward')
   write(fname,'(a)') '/export24/kyh/w1a.nc'
   call out3d(trim(fname),1,(/'w_up'/),wa_up+wa_dn,                    &
               'x',nxp,x/1000.,'z',nzp,z/1000.,'t',nt,t/60.,           &
               'w')
 end if
 ...
 if (coopt .eq. 1) then
   write(fname,'(a)') '/export24/kyh/kozcoup.nc'
   call out3d(trim(fname),1,(/'Co_up'/),cosd1,                         &
               'k',nkknew,kknew,'o',noonew,oonew,'z',nzp,z/1000.,      &
               'cospectrum upward')
   write(fname,'(a)') '/export24/kyh/kozcoresi.nc'
   call out3d(trim(fname),1,(/'Co_resi'/),cosd2,                         &
               'k',nkknew,kknew,'o',noonew,oonew,'z',nzp,z/1000.,      &
               'residual cospectrum')
   cosd1 = cosd1 + cosd2
   write(fname,'(a)') '/export24/kyh/kozco.nc'
