...
!-----------------------------------------------------
 ...
 integer, parameter :: fieldopt = 1, coopt = 0, psopt = 0, norm = 1, winopt = 0
 integer, parameter :: nx =1402, nz = 302, ntall = 480
 integer, parameter :: nxp =1200, nzp = 140, nt = 360
 integer, parameter :: xst = 100, zst = 30
...
!-----------------------------------------------------
 ...
 open(4,file='/usr/users/kyh/anal/work/rhoallapr')
 read(4,*) rhort
 close(4)
 open(5,file='/usr/users/kyh/arps/work/2ndgen/x12apr/rhobar')
 read(5,*) rhobar
 close(5)
 if (norm .eq. 0) then
   rhort = 1.
...
! read ubar or umean
 call opennc('/usr/users/kyh/result4/srcapr/UNbar/unbar.nc',ncid)
...
! read m^2
 call opennc('/export22/kyh/ing/x12apr/spec/msq/m2.nc',ncid)
...
print*, 'data reading'
 do n=1, nt
   ...
   open(1,file='/export24/kyh/ing/srcapr/bin/uprt.bin'//time,    &
          form='unformatted',convert='big_endian')
   read(1) udata
   close(1)
   open(2,file='/export24/kyh/ing/srcapr/bin/w.bin'//time,       &
          form='unformatted',convert='big_endian')
   read(2) wdata
   close(2)
