    ...
    real, dimension(nx) :: x
    real, dimension(ny) :: y
    real, dimension(nz) :: z, t0, u0, v0, nb, rho
    real, dimension(nt) :: h
    ...
    real, dimension(nx, ny, nz, nt, 3) :: var
    real, dimension(nt, nz, 3) :: var2
    ...
    double complex, allocatable, dimension(:,:,:) :: coef_up, coef_dn
...
if (omeh < 0. .or. j == ny/2+1)  CYCLE
        cc = omeh*omeh
        if ( cc > f0*f0*float(cf) .and. cc < 0.1*nb(k)*nb(k)/float(cf) ) then
          dd = (coefu(i,j,t)*freq_x(i)+coefv(i,j,t)*freq_y(j)) / &
               sqrt(freq_x(i)*freq_x(i)+freq_y(j)*freq_y(j)) *   &
               (0.,-1.)*nb(k)*t0(k)/g
          coef_up(i,j,t) = 0.5*(coeft(i,j,t) + dd) * &
              sqrt( abs(omeh)*freq_x(i)/sqrt(freq_x(i)**2+freq_y(j)**2)*1.e3 )
...
do k=1, nz
  var(:,:,k,:,:) = var(:,:,k,:,:)**2 * rho(k)/rho(35) / (nb(k)/nb(35))**3
enddo
do tt=1, 3
do t=1, nt
do k=1, nz
var2(t,k,tt) = sum(var(:,:,k,t,tt))/float(nx*ny)
enddo
enddo
enddo
...
!== Write Output ========================================================
    ...
    write(ncfilename,'(a,i0.0,a)') './tv_'//speriod//'_f',cf,'-e.nc'
    call out2d_yh(ncfilename, 3, (/'t_up','t_dn','all'/), &
               var2(:,35:,:), &
               'time'      , nt, h, &
               'height'    , nz-34, z(35:), &
               'T field')
