...
!   PURPOSE:
!
!   To calculate three-dimensional forward Fourier transform
!   (real space -> spectral space)
!
!   INPUT:
! 
...
!   data    : Three-dimensional data in real space (real).
!             Data should be grided at a constant grid spacing (regular grid).
!
!   OUTPUT:
!
!   coef    : Three-dimensional Fourier coefficients (double complex).
!
!----------------------------------------------------------------------------------

    subroutine fft3df(nx,ny,nz,dx,dy,dz,data,freqx,freqy,freqz,coef)

    integer                               ,intent(in)  :: nx     ,ny     ,nz
    real                                  ,intent(in)  :: dx     ,dy     ,dz
    real,            dimension(nx,ny,nz)  ,intent(in)  :: data
    real,            dimension(nx)        ,intent(out) :: freqx
    real,            dimension(ny)        ,intent(out) :: freqy
    real,            dimension(nz)        ,intent(out) :: freqz
    double complex  ,dimension(nx,ny,nz)  ,intent(out) :: coef

    integer                              :: i      ,j      ,k
    double precision, dimension(4*nx+15) :: workx
    double precision, dimension(4*ny+15) :: worky
    double precision, dimension(4*nz+15) :: workz
    double complex  , dimension(nx)      :: coefx
    double complex  , dimension(ny)      :: coefy
    double complex  , dimension(nz)      :: coefz
    ...
    do k=1,nz/2+1
      freqz(k) = float(k-1)/(nz*dz)
    end do
    do k=nz/2+2,nz
      freqz(k) = -freqz(nz-k+2)
    end do

    do k=1,nz
      do j=1,ny
        do i=1,nx
          coef(i,j,k) = dble(data(i,j,k))
        end do
      end do
    end do
    ...
    do j=1,ny
    do i=1,nx
      workz(1:4*nz+15) = 0.d0
      coefz(1:nz) = coef(i,j,1:nz)
      call cffti(nz,workz)
      call cfftf(nz,coefz,workz)
      coef(i,j,1:nz) = coefz(1:nz)
    end do
    end do
    ...
    do j=1,ny
      workx(1:4*nx+15) = 0.d0
      coefx(1:nx) = coef(1:nx,j,k)
      call cffti(nx,workx)
      call cfftf(nx,coefx,workx)
      coef(1:nx,j,k) = coefx(1:nx)
    end do
    end do
    ...
    do i=1,nx
      worky(1:4*ny+15) = 0.d0
      coefy(1:ny) = coef(i,1:ny,k)
      call cffti(ny,worky)
      call cfftf(ny,coefy,worky)
      coef(i,1:ny,k) = coefy(1:ny)
    end do
    end do
...
!   PURPOSE:
!
!   To calculate three-dimensional backward Fourier transform.
!   (spectral space -> real space)
!
!   INPUT:
! 
...
!----------------------------------------------------------------------------------
    ...
    subroutine fft3db(nx,ny,nz,coef,data)

    integer                               ,intent(in)  :: nx     ,ny     ,nz
    double complex  ,dimension(nx,ny,nz)  ,intent(in)  :: coef
    ...
    double complex  , dimension(ny)       :: coefy
    double complex  , dimension(nz)       :: coefz
    double complex  , dimension(nx,ny,nz) :: coef2
    ...
    do j=1,ny
      workx(1:4*nx+15) = 0.d0
      coefx(1:nx) = coef2(1:nx,j,k)
      call cffti(nx,workx)
      call cfftb(nx,coefx,workx)
      coef2(1:nx,j,k) = (1./float(nx))*coefx(1:nx)
    end do
    end do
    ...
    do i=1,nx
      worky(1:4*ny+15) = 0.d0
      coefy(1:ny) = coef2(i,1:ny,k)
      call cffti(ny,worky)
      call cfftb(ny,coefy,worky)
      coef2(i,1:ny,k) = (1./float(ny))*coefy(1:ny)
    end do
    end do
    ...
    do i=1,nx
      workz(1:4*nz+15) = 0.d0
      coefz(1:nz) = coef2(i,j,1:nz)
      call cffti(nz,workz)
      call cfftb(nz,coefz,workz)
      coef2(i,j,1:nz) = (1./float(nz))*coefz(1:nz)
    end do
    end do
