...
! LOOP
  ...
  fname1 = trim(file_i)//'/'//timec//'/'//trim(expname)//'.std_p.'//timec//'+'//hhh0//'.nc'
  fname2 = trim(file_i)//'/'//timec//'/'//trim(expname)//'.std_p.'//timec//'+'//hhh1//'.nc'
...
! END
  ...
  SUBROUTINE getvars
    ...
    fname = trim(file_i)//'/'//timec//'/'//trim(expname)//'.std_p.'//timec//'+'//hhh//'.nc'
    ...
    call geta4d(ncid,trim(ivarname(1)),1,nxr,1,nyr  ,1,nzr,1,1, u  (:, :   ,:,ni))
    call geta4d(ncid,trim(ivarname(2)),1,nxr,1,nyr-1,1,nzr,1,1, v  (:,2:nyr,:,ni))
    call geta4d(ncid,trim(ivarname(3)),1,nxr,1,nyr  ,1,nzr,1,1, w  (:, :   ,:,ni))
    call geta4d(ncid,trim(ivarname(4)),1,nxr,1,nyr  ,1,nzr,1,1, pt (:, :   ,:,ni))
    call geta4d(ncid,trim(ivarname(5)),1,nxr,1,nyr  ,1,nzr,1,1, gph(:, :   ,:,2 ))
    call geta1d(ncid,'t',1,1,t3(2))

    order_dt = 2
    if ( ex1 ) then
      call opennc(trim(fname1),ncid2)
      call geta4d(ncid2,trim(ivarname(5)),1,nxr,1,nyr,1,nzr,1,1, gph(:,:,:,1))
