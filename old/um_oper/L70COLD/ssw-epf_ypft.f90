...
! DUMP
  ...
  do iw=1, nwn
    ...
    if (l_ftail) then
      write(fname,'(a,i6.6,a,i0.1,a)') trim(file_o)//'/'//trim(expname)//'.'// &
         trim(fntype)//'.',yyyymm,trim(fnamet(2))//'_av',int(fct_intg*24.),'h.nc'
    else
      write(fname,'(a,i6.6,a,i0.1,a)') trim(file_o)//'/'//trim(expname)//'.'// &
         trim(fntype)//'.',yyyymm,'_av',int(fct_intg*24.),'h.nc'
    end if
