PROGRAM analysis_UM
  ...
  real, dimension(:,:,:),     allocatable ::  um, vres, wres, ptm
  ...
  ! get var.s
  allocate( u_in(nx,ny,nz,nt_in), v_in(nx,ny,nz,nt_in), w_in(nx,ny,nz,nt_in) )
  allocate( pt_in(nx,ny,nz,nt_in) )
  call get4d(ncid,trim(ivarname(1)),nx,ny,nz,nt_in, u_in )
  call get4d(ncid,trim(ivarname(2)),nx,ny,nz,nt_in, v_in )
  call get4d(ncid,trim(ivarname(3)),nx,ny,nz,nt_in, w_in )
  call get4d(ncid,trim(ivarname(4)),nx,ny,nz,nt_in, pt_in)
  do n=1, nt
    n_in2 = n*t_interval
    ...
    call tempo_avg(nx,ny,nz,t_interval,missv,w_in (:,:,:,n_in1:n_in2), w (:,:,:,n))
    call tempo_avg(nx,ny,nz,t_interval,missv,pt_in(:,:,:,n_in1:n_in2), pt(:,:,:,n))
  enddo
  ...
  call zonal_avg(nx,ny,nz,nt,missv,w , wres)
  call zonal_avg(nx,ny,nz,nt,missv,pt, ptm )
  do n=1, nt
  do k=1, nz
  do j=1, ny
    u (:,j,k,n) = u (:,j,k,n) - um  (j,k,n)
    v (:,j,k,n) = v (:,j,k,n) - vres(j,k,n)
    w (:,j,k,n) = w (:,j,k,n) - wres(j,k,n)
...
!  enddo
  ...
  ! flux terms
  call corr_zonal_avg(nx,ny,nz,nt,missv,v,pt, vflx)
  call corr_zonal_avg(nx,ny,nz,nt,missv,w,pt, wflx)

  ! residual mean meridional circulations
  call gradz_2nd(1,ny,nz,nt,ptm,zp,missv, stabz)
  do n=1, nt
  do k=1, nz
  do j=1, ny
  ...
  do k=1, nz
  do j=1, ny
    if ( vptm(j,k,n) /= missv .and. stabz(j,k,n) /= missv ) then
      term1(j,k,n) = vptm(j,k,n)/stabz(j,k,n)
    else
      term1(j,k,n) = missv
    end if
  enddo
