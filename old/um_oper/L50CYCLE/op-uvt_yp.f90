...
PROGRAM analysis_UM
  ...
  if (idt*iut*ifc == 1)  allocate( set(iv)%var_out(nxa,nya,nza,nta) )
  ...
  if (idt*iut*ifc == 1)  allocate( var_za(1,nyr,nzr,ntr) )
  ...
  call tempo_avg(1,nyr,nzr,i_time,missv,var_za(1,:,:,:i_time), set(iv)%var_out)
  deallocate( var_za )
