PROGRAM ZME2_RA
  ...
  character(len=64)  ::  ivarname(nv), ivarnamer(nv), ovarname(nvo)
  character(len=128) ::  f_namelist

  integer ::  yyyymm, date(2), utc(12), xy_itv(2)
  real    ::  fct(2), fct_itv
  character(len=10)  ::  expname, exception(100)
...
! LOOP
  ...
  select case ( iv )
  case ( 1 )
    allocate( var2(nxr,nyr,nzr,1) )
    call geta4d(ncid, trim(ivarname(iv)),1,nxr,1,nyr,1,nzr,it,1, var2)
    do k=1, nzr
    do j=1, nyr
      do i=2, nxr
  ...
  ! get / interporate ERA
  allocate( vart(nxn,nyn,nzr,1) )
  vart(:,:,:,1) = missv

  allocate( var2(nxa,nya,nza,1) )
  if (iv < 5) then
    i4 = (/1,1,1,ita/)  ;  n4 = (/nxa,nya,nza,1/)
    i4_inv = (/0,1,1,0/)
  else
    i4 = (/1,1,ita,1/)  ;  n4 = (/nxa,nya,1,1/)
    i4_inv = (/0,1,0,0/)
  end if
  call get_era(ncid2,ivarnamer(iv),i4,n4,i4_inv, var2)
  if (iv == 4)  var2(:,:,:,1) = var2(:,:,:,1) / g
  do k=1, nzr
    if (iz(k) /= 0)  &
       call interp(nxa,nya,var2(:,:,iz(k),1),nxn,nyn, vart(:,:,k,1))
  enddo
  deallocate( var2 )
  ...
  do k=1, nzr
    if (iz(k) /= 0)  &
       call zm_err2_itv(nxr,nyr,1,xy_itv(1),xy_itv(2),1,var(:,:,k,1),vart(:,:,k,1), &
...
! DUMP
  ...
  call outnc(trim(file_o),nvo,set,'ZME2_RA')

! END

  deallocate( t_fct, t )
  deallocate( lon, lat, p, lonu, latv, lona, lata, pa, iz )
  ...
  SUBROUTINE getfname
    ...
    if (iv < 5) then
      if (fct_h > 24) then
        fname = trim(file_i(1))//'/'//timec//'/'//timec//'.press2.nc'
      else
        fname = trim(file_i(1))//'/'//timec//'/'//timec//'.press1.nc'
      end if
      write(ftrue,'(a,i4.4,i2.2,a)') trim(file_i(2))//'/ERA.',tyear,tmonth,'.press.nc'
    else
      fname = trim(file_i(1))//'/'//timec//'/'//timec//'.sfc.nc'
      write(ftrue,'(a,i4.4,i2.2,a)') trim(file_i(2))//'/ERA.',tyear,tmonth,'.pmsl.nc'
    end if
  ...
  SUBROUTINE getdim
    ...
    integer                            ::  ka
    integer, dimension(:), allocatable ::  pa_i
    real,    dimension(:), allocatable ::  lata_i
    ...
    ! ERA dim.
    call dilen(ncid2,'longitude', nxa,l_err)
    call dilen(ncid2,'latitude' , nya,l_err)
    call dilen(ncid2,'levelist' , nza,l_err)
    allocate( lona(nxa), lata(nya), pa(nza) )
    allocate( lata_i(nya), pa_i(nza) )
    call get1d(ncid2,'longitude',nxa, lona  )
    call get1d(ncid2,'latitude' ,nya, lata_i)
    call iget1d(ncid2,'levelist',nza, pa_i)
    do j=1, nya
      lata(j) = lata_i(nya-j+1)
    enddo
    do k=1, nza
      pa(k) = float(pa_i(nza-k+1))
    enddo
    deallocate( lata_i, pa_i )
    allocate( iz(nz) )
    do k=1, nz
      iz(k) = 0
      do ka=1, nza
        if (abs(p(k)-pa(ka)) < min(p(k),pa(ka))*1.e-3)  iz(k) = ka
      enddo
    enddo
  ...
  SUBROUTINE gettid
    ...
    if (mod(tyear,4) == 0)  mondate(2) = 29

    call timeinfo(ncid, nt0)
    allocate( time0(nt0) )
    call get1d(ncid,'t',nt0, time0)
    do n=1, nt0
      if ( abs(time0(n) - t_fct(ifct)) < v_small )  it = n
    enddo
    deallocate( time0 )

    ! ERA tid
    call dilen(ncid2,'time', nta,l_err)
    dta = 24*mondate(tmonth)/nta
    ita = ( (tdate-1)*24 + thour ) / dta + 1
...
SUBROUTINE zm_err2_itv(nx,ny,nt,itvx,itvy,itvt,var,var0, zme,zmse)

  implicit none

  integer,                   intent(in) ::  nx, ny, nt, itvx, itvy, itvt
  real, dimension(nx,ny,nt), intent(in) ::  var
  real, dimension(nx/itvx,(ny-1)/itvy+1,nt/itvt), &
                             intent(in) ::  var0
...
END subroutine zm_err2_itv


SUBROUTINE get_era(ncid,varname,i4,n4,i4_inv, dera)
  ...
  integer, dimension(4), intent(in) ::  i4, n4, i4_inv
  character(len=*),      intent(in) ::  varname

  real, dimension(n4(1),n4(2),n4(3),n4(4)), intent(out) ::  dera

  integer ::  st, varid, i,j,k,n
  real*8  ::  sf, ao

  integer ::  ii(n4(1)), jj(n4(2)), kk(n4(3)), nn(n4(4))
  integer*2, dimension(n4(1),n4(2),n4(3),n4(4)) ::  dera4, derar
  ...
  call sgeta4d(ncid,trim(varname),i4(1),n4(1),i4(2),n4(2),i4(3),n4(3),i4(4),n4(4), derar)

  ii(:) = (/ (i, i=1, n4(1)) /)
  jj(:) = (/ (j, j=1, n4(2)) /)
  kk(:) = (/ (k, k=1, n4(3)) /)
  nn(:) = (/ (n, n=1, n4(4)) /)
  if ( i4_inv(1) )  ii(:) = (/ (n4(1)-i+1, i=1, n4(1)) /)
  if ( i4_inv(2) )  jj(:) = (/ (n4(2)-j+1, j=1, n4(2)) /)
  if ( i4_inv(3) )  kk(:) = (/ (n4(3)-k+1, k=1, n4(3)) /)
  if ( i4_inv(4) )  nn(:) = (/ (n4(4)-n+1, n=1, n4(4)) /)

  dera4(:,:,:,:) = derar(ii(:),jj(:),kk(:),nn(:))

  dera = real(float(dera4)*sf+ao)

  RETURN

END subroutine get_era
