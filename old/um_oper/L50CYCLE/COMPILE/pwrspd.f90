...
!
!           2006: Added PSD2D
!   11 May, 2007: modified PSD1D (add winopt, eval)
!   24 May, 2007: modified PSD at Nyquist frequency
!
!
!   REQUIRED MODULE:
!
...
!----------------------------------------------------------------------------------
    ...
    subroutine psd1d(ndata,ma   ,x    ,dx   ,data ,wavk ,pwsd ,winopt,eval )
...
!-------------------------------------------------------------------------------
    ...
    integer,                       intent(in)  :: ndata ,ma    ,winopt,eval
    real,                          intent(in)  :: dx
...
!++ 6. Calculate power spectral density
    ...
    if (ndata/2*2 .eq. ndata)  pwsd(ndata/2+1) = pwsd(ndata/2+1) / 2.
...
!++ PSD ( & sorting )
    ...
    if (nx/2*2 .eq. nx)  pwsd(nx/2+1,:) = pwsd(nx/2+1,:) / 2.
    if (ny/2*2 .eq. ny) then
      pwsd(:,1)        = pwsd(:,1)        / 2.
      pwsd(:,ny/2*2+1) = pwsd(:,ny/2*2+1) / 2.
    end if
    if (nx/2*2.eq.nx .and. ny/2*2.eq.ny) then
      pwsd(nx/2+1,1)        = pwsd(nx/2+1,1)        * 2.
      pwsd(nx/2+1,ny/2*2+1) = pwsd(nx/2+1,ny/2*2+1) * 2.
    end if
