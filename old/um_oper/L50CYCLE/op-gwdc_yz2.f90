...
PROGRAM analysis_UM
  ...
  integer, parameter ::  dstart = 2, dend = 19   ! 6, 31
  integer, parameter ::  nutc  =  1, ustart =12
  real,    parameter ::  fstart = 1.0, fend = 1.0, fitv = 1.0   ! [day]
  integer, parameter ::  nvar = 1!6
...
!  data                   ivarname(2) /'unspecified_1'/
!  data                   ivarname(3) /'unspecified_2'/
!  data                   ivarname(4) /'unspecified_3'/
!  data                   ivarname(5) /'unspecified_4'/
!  data                   ivarname(6) /'unspecified_5'/
  ! output variables
...
!  data                   ovarname(6) /'mf_s'/
  ...
  set(1)%var_out(1,:,:,1) = var_za(1,:,:,i_time)

  write(fname,'(a,i1,a)') trim(ofdir)//trim(expname)//'/anal/'// &
          trim(expname)//'.'//trim(outname)//'.'//cutc0//'+',int(fct/24.),'.nc'
