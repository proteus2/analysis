...
!  character(len=64)  ::  ivfile(nvar), ivarname(nvar), ovarname(nvar)
  ...
  integer, parameter ::  dstart = 1, dend =  1
...
!  if ( ifc == 1 .or. ifc == nfct )  avgcoef = avgcoef*0.5
  ...
  fname = '/data11/kyh/UM_case/ctl/2010010712/std.gph_p.2010010712+'
  if (fct <= 24) then
    fname = trim(fname)//'024.nc'
  else if (fct <= 48) then
    fname = trim(fname)//'048.nc'
  else if (fct <= 72) then
    fname = trim(fname)//'072.nc'
  else if (fct <= 96) then
    fname = trim(fname)//'096.nc'
  else
    fname = trim(fname)//'120.nc'
  end if
