PROGRAM TEM
  ...
  integer ::  yyyymm, date(2), utc(12), nf_intg(999), itvy
  real    ::  fct(2), fct_intg
  character(len=10)  ::  expname, exception(99)
  character(len=128) ::  file_i, file_o

  namelist /ANALCASE/ EXPNAME, YYYYMM
  namelist /PARAM/ DATE, UTC, FCT, FCT_INTG, EXCEPTION, NF_INTG, ITVY
...
! END
  ...
  SUBROUTINE getvars
    ...
    real, dimension(nxr,ny00,nzr,nf_intg(ifct)) ::  u00, v00, w00, pt00
    ...
    order_dt = 2
    if (it /= 1) then
      call geta4d(ncid,trim(ivarname(5)),1,nxr,1,ny00,1,nzr,it-1,1, gph(:,:,:,1))
      ...
      call geta4d(ncid2,trim(ivarname(5)),1,nxr,1,ny00,1,nzr,nf1,1, gph(:,:,:,1))
      ...
      call geta4d(ncid2,trim(ivarname(5)),1,nxr,1,ny00,1,nzr,1,1, gph(:,:,:,3))
