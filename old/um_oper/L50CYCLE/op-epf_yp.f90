...
PROGRAM analysis_UM
  ...
  integer ::  iz, it
  integer ::  i,j,k,n,iv, ncid, tag, iwn1, iwn2
  ...
  do n=1, 8
  do k=1, nzr
    tem_in(:,:,k,n) = tem_in(:,:,k,n)*g*h_scale/rd/(tem_in2(:,:,k,n)*(p(k)/1.e5)**kappa)
  enddo
  enddo
...
!  call grady_2nd(1,ny,nz,1,ptm,lat,missv, grady)
  ...
  do i=1, nxa
    if (epd(j,k) /= missv) then
      nepd(j,k) = nepd(j,k) + 1
      set(1)%var_out(i,j,k,1) = set(1)%var_out(i,j,k,1) + epd(j,k)
    end if
    if (fy(j,k) /= missv) then
      nfy(j,k) = nfy(j,k) + 1
      set(2)%var_out(i,j,k,1) = set(2)%var_out(i,j,k,1) + fy(j,k)
    end if
    if (fz(j,k) /= missv) then
      nfz(j,k) = nfz(j,k) + 1
      set(3)%var_out(i,j,k,1) = set(3)%var_out(i,j,k,1) + fz(j,k)
    end if
  enddo
  enddo
  enddo
