...
PROGRAM analysis_UM
  ...
  data                   ivfile(3) /'fu_p'/
  ! input variables
  data                   ivarname(1) /'field68'  /
  data                   ivarname(2) /'field1596'/
  data                   ivarname(3) /'field424' /
  ! output variables
  data                   ovarname(1) /'fu_gwdo'/
  data                   ovarname(2) /'fu_bldo'/
  data                   ovarname(3) /'fu_ussp'/
  ...
  call zonal_avg(nxr,ny,nzr,1,missv,var, set(iv)%var_out(1,:,:,ifc))
  deallocate ( var )
