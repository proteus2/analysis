...
!--------------------------------------------------------------------------------------------
      ...
      write(ifname,'(a,a2,a2,a2,a)') &
             '/export9/radiosonde/rusa/osan31_100/rawdata/asc/02',arg1,arg2,arg3,'.prn'
      print *, ifname
      open (11,file=ifname,form='formatted')
      do i=1,num
        read (11, '(2x,f4.1,11x,f5.1,12x,f4.1,4x,f4.1)', iostat=istat ) &
           p(i),t(i),deg(i),w(i)
      ...
      if ( nlev .gt. 20 ) then      ! IF DATA NUMBER IS SMALLER THAN 20, STOP PROGRAM
        do i=1,nlev
          if (deg(i).eq.-99.9 .or. deg(i).eq.-999.0) then
            deg(i)=MIS
          end if
          if (w(i).eq.-99.9 .or. w(i).eq.-999.0) then
            w(i)=MIS
          end if
          if (p(i).eq.-99.9 .or. p(i).eq.-999.0) then
            p(i)=MIS
          end if
          if (t(i).eq.-99.9 .or. t(i).eq.-999.0) then
            t(i)=MIS
          end if
        end do
...
!-------------------------------------------------------------------------------------------- 
! DUE TO PRESSURE, NEED TO MODIFY PRESSURE OF TEMPERATURE 
!--------------------------------------------------------------------------------------------

        ct = 100
        do j=1,zz-1
          if (pmt(j) .lt. pmt(j+1) ) then
            id=j
            if (ct .gt. id) then
              ct=id
              print *,ct
...
!--------------------------------------------------------------------------------------------  
! DUE TO PRESSURE, NEED TO MODIFY PRESSURE OF WIND
!-------------------------------------------------------------------------------------------- 
          
        ct = 100
         do j=1,ww-1
           if (pmw(j) .lt. pmw(j+1) ) then
             id=j
             if (ct .gt. id) then
               ct=id
             end if
