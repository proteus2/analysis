...
!------------------------------------------------------------------------
!
!   This module requires subroutines rgrd1 and rgrd2
!   RGRD1 and RGRD2 are subroutines in REGRID PACK, and can be downloaded
!   in http://www.scd.ucar.edu/softlib/mathlib.html
!
!   Author
!   In-Sun Song
!
...
!------------------------------------------------------------------------                
    ...
    subroutine int1d(n,x,f,m,y,g,intpol,bad)

!------------------------------------------------------------------------
!
!   Intepolation 1-dimensional data f to another 1-dimensional grid (y)
...
!------------------------------------------------------------------------
    ...
    real, dimension(m), intent(inout) :: g
    real,               intent(in)    :: bad 

    integer :: i,l,lw,liw,ier,length
    integer :: istrt, iend
    logical :: incx, incy
    integer, allocatable, dimension(:)  :: iw
    real(r8), dimension(n) :: xd
    real(r8), dimension(n) :: fd
    real(r8), allocatable, dimension(:) :: yd
    real(r8), allocatable, dimension(:) :: gd
    real(r8), allocatable, dimension(:) :: w
   
    call index(n,x,m,y,incx,incy,istrt,iend)

    print *, incx, incy, istrt, iend

    liw = m+1
    if (intpol == 1) then
      lw = m+1 
    else if (intpol == 3 ) then
      lw = 4*m+1
    else
    ...
    allocate(iw(1:liw))
    allocate(w(1:lw))
    allocate(yd(1:iend-istrt+1))
    allocate(gd(1:iend-istrt+1))
    ...
    call rgrd1(n,xd,fd,iend-istrt+1,yd,gd,intpol,w,lw,iw,liw,ier)
