...
C     MODULE QUADPACK
C
C     Variables below are needed to evaluate function f. Therefore,
C     User should change variables if variables required to calculate
C     the function f are different from the variables below
C
C-----------------------------------------------------------------------

c     double precision  stn,trn,ust,zb,zt

      contains

c-----------------------------------------------------------------------
c
c     SUBROUTINE NINTEG1D
c
c     Purpose:
c
c     To integrate a 1-dimensional function numerically using the 
c     subroutine DQAG in the QUADPACK.
c
c     Author:    
c
c     In-Sun Song
c     Laboratory for mesoscale dynamics, Yonsei Univ., Seoul, Korea
c
c-----------------------------------------------------------------------

      subroutine ninteg1d(f,a,b,epsabs,epsrel,key,result,abserr,ier)
      ...
      integer limit,lenw,last,neval
      integer, dimension(:), allocatable :: iwork
      double precision, dimension(:), allocatable :: work

      external f
c
c     User may change a value of 'limit', which determines the maximum
c     number of subintervals in the partition of the given integration
c     interval (a,b). 'limit' should be larger than or equal to 1.
c
c      limit = 10000
      limit = 2000000 
...
c                     a given definite integral I = integral of f over
c                     (a,b), hopefully satisfying following claim for
c                     accuracy  abs(I-result) .le. max(epsabs,epsrel*abs(I)).
c     Description   
c
...
c                     a given definite integral  I = integral of f over ,
c                     (a,b), hopefully satisfying following claim for
c                     accuracy abs(I-reslt).le.max(epsabs,epsrel*abs(I)).
c     Description   
c
c        computation of a definite integral
c        standard fortran subroutine
...
c     Author        : Piessens,robert,appl. math. & progr. div. - k.u.leuven
c                   : De doncker,elise,appl. math. & progr. div. - k.u.leuven
c     Purpose       : This routine maintains the descending ordering in 
c                     the list of the local error estimated resulting 
c                     from the interval subdivision process. At each 
c                     call two error estimates are inserted using the
c                     sequential search method, top-down for the largest
c                     error estimate and bottom-up for the smallest error
c                     estimate.
c     Description
c
c           ordering routine
...
c                     estimate. J = integral of abs(f) over (a,b)
c
c     Description
c
c           integration rules
...
c                     De doncker,elise,appl. math. & progr. div. - k.u.leuven
c     Purpose       : To compute I = integral of f over (a,b), with error
c                     estimate and J = integral of abs(f) over (a,b).
c     Description
c
c           integration rules
c           standard fortran subroutine
