MODULE tem
  ...
  character(len=64), dimension(10) ::  varname_tem_qg =                  &
      (/'v_res  ','w_res  ','cor    ','epd    ','epd_y  ',               &
        'epd_z  ','f_y    ','f_z    ','tadv_z ','u_force'/)
  character(len=64), dimension(12) ::  varname_tem_z =                   &
      (/'v_res  ','w_res  ','cor    ','uadv_y ','uadv_z ',               &
        'epd    ','epd_y  ','epd_z  ','f_y    ','f_z    ',               &
        'u_force','ru_eddy'/)
!  data  varname_tem_p(1 ) /'v_res'  /
!  data  varname_tem_p(2 ) /'w_res'  /
!  data  varname_tem_p(3 ) /'cor'    /
!  data  varname_tem_p(4 ) /'uadv_y' /
!  data  varname_tem_p(5 ) /'uadv_z' /
!  data  varname_tem_p(6 ) /'epd'    /
!  data  varname_tem_p(7 ) /'f_y'    /
!  data  varname_tem_p(8 ) /'f_z'    /
!  data  varname_tem_p(9 ) /'tadv_y' /
!  data  varname_tem_p(10) /'tadv_z' /
!  data  varname_tem_p(11) /'t_eddy' /
!  data  varname_tem_p(12) /'u_force'/
!  data  varname_tem_p(13) /'t_force'/
...
SUBROUTINE tem_hydro_p(                                                  &
  ...
  call grady_2nd(ny,nz,phi*coslat,lat, grady)
  call gradz_2nd_irr(ny,nz,rvpt,zp, gradz)
  vres(:,:) = vres(:,:) - ( gradz(:,:)-rvpt(:,:)*dptmd2z(:,:)/dptmdz(:,:) ) &
              /dptmdz(:,:)/rho0(:,:)
  do k=1, nz
  do j=2, ny-1
    wres(j,k) = wres(j,k) + grady(j,k)/rho0(j,k)/coslat(j,k)
  enddo
  enddo
...
SUBROUTINE tem_qg(                                                       &
     ...
     vres,wres,cor,epd,epd_y,epd_z,fy,fz,tadvz,u_force )
  ...
  real, dimension(ny,nz) ::  gpm, dpt0dz, dpt0d2z
  real, dimension(ny,nz) ::  gradz, temp
  ...
  call set_gridvar_qg(ny,nz,lat,p,lat0,h_scale)
...
!    call fdcoef(1,2,zp(k),zp(k-1:k+1), coef(:))
  ...
  dpt0dz(:,:) = spread( coefz(:)*(kappa/h_scale*grad1(:) + grad2(:)),    &
                        1,ny )
  dpt0d2z(:,:) = kappa/h_scale*dpt0dz(:,:) +                             &
                 spread( coefz(:)*(kappa/h_scale*grad2(:) + grad3(:)),   &
                         1,ny )
  call missing_bdy(ny,nz,dpt0dz,missv,0,0,2,1)
  call missing_bdy(ny,nz,dpt0d2z,missv,0,0,3,2)
  ...
  enddo
  call gradz_2nd_irr(ny,nz,rvpt,zp, gradz)
  temp(:,:) = ( gradz(:,:)-rvpt(:,:)*dpt0d2z(:,:)/dpt0dz(:,:) )/dpt0dz(:,:)
  vres(:,:) = vm(:,:) - temp(:,:)/rho0(:,:)
...
SUBROUTINE tem_z(                                                        &
  ...
  ! residual mean meridional circulation
  temp(:,:) = rcos(:,:)*phi(:,:)
  call grady_2nd(ny,nz,temp,lat, grady)
  call gradz_2nd_irr(ny,nz,temp,z, gradz)
  do k=1, nz
  do j=2, ny-1
    vres(j,k) = (rvm(j,k) - gradz(j,k)/rcos(j,k))/rhom(j,k)
  enddo
  enddo
  do k=1, nz
  do j=2, ny-1
    wres(j,k) = (rwm(j,k) + grady(j,k)/rcos(j,k))/rhom(j,k)
  enddo
  enddo
...
SUBROUTINE tem_z_cp(                                                     &
  ...
  temp(:,:) = r_t(:,:)*coslat(:,:)*phi(:,:)
  call grady_2nd(ny,nz,temp,lat, grady)
  call gradz_i2m(ny,nz,temp,zt , gradz)
  do k=1, nz
  do j=2, ny-1
    vres(j,k) = (rvm(j,k) - gradz(j,k)/rcos(j,k))/rhom(j,k)
  enddo
  ...
  do k=1, nz
  do j=2, ny-1
    rwm(j,k) = rwm(j,k) + grady(j,k)/(r_t(j,k)*coslat(j,k))
  enddo
  enddo
  ...
  epd_z(:,:) = -(druwr3dz(:,:) + gradz(:,:))/r3rho(:,:) * 86400.
  call missing_bdy(ny,nz,epd_z,missv,1,1,2,1)
...
SUBROUTINE set_gridvar_qg(ny,nz,lat,p,lat0,h_scale)
